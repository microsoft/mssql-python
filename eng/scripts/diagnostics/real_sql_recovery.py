#!/usr/bin/env python3
"""Run private Unix process checks and five approved SQL2025 recovery scenarios.
...
Use --process-only to run process checks without Docker (--owner is optional).
...
def nonreaping_runtime_diagnostic(module):
    module.ChildProcess.check_platform()
    process = subprocess.Popen(
    ...
    child = module.ChildProcess(process)
    try:
        require(child.wait(5) == 23, "Non-reaping wait returned an incorrect exit status")
        flags = os.WEXITED | os.WNOHANG | os.WNOWAIT
        first = os.waitid(os.P_PID, process.pid, flags)
        second = os.waitid(os.P_PID, process.pid, flags)
        require(first is not None and second is not None, "WNOWAIT did not retain wait status")
        require(
            first.si_pid == second.si_pid == process.pid
            and first.si_code == second.si_code == os.CLD_EXITED
            and first.si_status == second.si_status == 23,
            "Repeated WNOWAIT observations changed identity/status",
        )
        require(process.returncode is None, "Exit observation prematurely reaped the Popen child")
        require(child.reap(1) == 23 and child.reaped, "Final child reap failed")
        require(child.reap(0) == 23 and process.returncode == 23, "Completed reap was not stable")
        try:
            os.waitid(os.P_PID, process.pid, flags)
        except ChildProcessError:
            pass
        else:
            raise RuntimeError("Child remained waitable after the final successful reap")
        return {
            "case": "runtime-wnowait-retains-identity-final-reap",
            "success": True,
            "python": sys.version.split()[0],
            "platform": sys.platform,
            "wnowait": os.WNOWAIT,
            "repeat_nonreaping_observations": 2,
            "successful_reaps": 1,
        }
    finally:
        if not child.reaped:
            cleanup = module.Commands.stop(child, time.monotonic() + 4)
            if not cleanup.reaped or cleanup.errors:
                print(
                    "[process-probe] Runtime capability probe cleanup incomplete", file=sys.stderr
                )
...
def process_diagnostics(helper, root):
    ...
    results = [nonreaping_runtime_diagnostic(module)]
    ...
    original_stop = module.Commands.stop
    held_leader_checks = []

    def check_held_leader(child, deadline):
        require(child.observed_code == 0, "Finite launcher exit was not observed separately")
        require(
            child.process.returncode is None, "Finite launcher was reaped before group teardown"
        )
        held_leader_checks.append(True)
        return original_stop(child, deadline)

    try:
        module.Commands.stop = staticmethod(check_held_leader)
        started = time.monotonic()
        try:
        ...
        require(held_leader_checks == [True], "Held-leader teardown path was not exercised")
        results.append(
            {
                "case": "hung-descendant-control-survives",
                "success": True,
                "leader_unreaped_before_teardown": True,
            }
        )
    finally:
        module.Commands.stop = staticmethod(original_stop)
        if ready.exists():
            stop_known(int(ready.read_text()))
        control.terminate()
