...
def pytest_collection_modifyitems(items):
    ...
    for key, node in classes.items():
        ...
        if len(markers) != 1 or len(markers[0].args) != 1 or type(markers[0].args[0]) is not bool:
            raise pytest.UsageError(f"Expected one boolean QEMU exclusion on {key}")
        selected.append((key, node, markers[0]))
    for key, node, marker in selected:
        node.own_markers.remove(marker)
        STATE["removed"].append({"file": key[0], "class": key[1], "condition": marker.args[0]})
    STATE["collected"] = actual
    save()
