...
def test_bit_bulkcopy():
    ...
    cursor.execute(f"SELECT id, bit_value, is_active, is_deleted, description FROM {table_name} ORDER BY id")
    ...
    for i, row in enumerate(rows):
        print(f"  ID: {row[0]}, BIT: {row[1]}, IS_ACTIVE: {row[2]}, IS_DELETED: {row[3]}, Description: {row[4]}")
        
        assert row[0] == expected_values[i][0], f"ID mismatch at row {i}"
        assert row[1] == expected_values[i][1], f"BIT value mismatch at row {i}: expected {expected_values[i][1]}, got {row[1]}"
        assert row[2] == expected_values[i][2], f"IS_ACTIVE mismatch at row {i}: expected {expected_values[i][2]}, got {row[2]}"
        assert row[3] == expected_values[i][3], f"IS_DELETED mismatch at row {i}: expected {expected_values[i][3]}, got {row[3]}"
    ...
    assert all_false[0] == False and all_false[1] == False and all_false[2] == False, f"All False verification failed"
    ...
    assert all_true[0] == True and all_true[1] == True and all_true[2] == True, f"All True verification failed"
