...
def test_bulkcopy_more_columns_than_table(cursor):
    ...
    cursor.execute(f"CREATE TABLE {table_name} (id INT PRIMARY KEY, value1 INT, value2 INT)")
...
def test_bulkcopy_fewer_columns_than_table(cursor):
    ...
    cursor.execute(f"IF OBJECT_ID('{table_name}', 'U') IS NOT NULL DROP TABLE {table_name}")
    cursor.execute(f"CREATE TABLE {table_name} (id INT PRIMARY KEY, value1 INT, value2 INT NULL)")
    cursor.connection.commit()
...
def test_bulkcopy_column_reordering(cursor):
    ...
    assert (
        rows[2][0] == 3 and rows[2][1] == "Carol" and rows[2][2] == 35 and rows[2][3] == "Chicago"
    )
