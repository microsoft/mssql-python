...
Unit tests for connection string normalization methods in _ConnectionStringParser.
"""
...
class Test_ConnectionStringAllowList:
    """Unit tests for connection string normalization in _ConnectionStringParser."""
    ...
    def test_normalize_key_server(self):
        ...
        assert _ConnectionStringParser.normalize_key('server') == 'Server'
        assert _ConnectionStringParser.normalize_key('SERVER') == 'Server'
        assert _ConnectionStringParser.normalize_key('Server') == 'Server'
        assert _ConnectionStringParser.normalize_key('address') == 'Server'
        assert _ConnectionStringParser.normalize_key('ADDRESS') == 'Server'
        assert _ConnectionStringParser.normalize_key('addr') == 'Server'
        assert _ConnectionStringParser.normalize_key('ADDR') == 'Server'
    
    def test_normalize_key_authentication(self):
        """Test normalization of authentication parameters."""
        assert _ConnectionStringParser.normalize_key('uid') == 'UID'
        assert _ConnectionStringParser.normalize_key('UID') == 'UID'
        assert _ConnectionStringParser.normalize_key('pwd') == 'PWD'
        assert _ConnectionStringParser.normalize_key('PWD') == 'PWD'
        assert _ConnectionStringParser.normalize_key('authentication') == 'Authentication'
        assert _ConnectionStringParser.normalize_key('trusted_connection') == 'Trusted_Connection'
    
    def test_normalize_key_database(self):
        """Test normalization of database parameter."""
        assert _ConnectionStringParser.normalize_key('database') == 'Database'
        assert _ConnectionStringParser.normalize_key('DATABASE') == 'Database'
        # 'initial catalog' is not in the restricted allowlist
        assert _ConnectionStringParser.normalize_key('initial catalog') is None
    
    def test_normalize_key_encryption(self):
        """Test normalization of encryption parameters."""
        assert _ConnectionStringParser.normalize_key('encrypt') == 'Encrypt'
        assert _ConnectionStringParser.normalize_key('trustservercertificate') == 'TrustServerCertificate'
        assert _ConnectionStringParser.normalize_key('hostnameincertificate') == 'HostnameInCertificate'
        assert _ConnectionStringParser.normalize_key('servercertificate') == 'ServerCertificate'
    def test_normalize_key_connection_params(self):
        """Test normalization of connection behavior parameters."""
        assert _ConnectionStringParser.normalize_key('connectretrycount') == 'ConnectRetryCount'
        assert _ConnectionStringParser.normalize_key('connectretryinterval') == 'ConnectRetryInterval'
        assert _ConnectionStringParser.normalize_key('multisubnetfailover') == 'MultiSubnetFailover'
        assert _ConnectionStringParser.normalize_key('applicationintent') == 'ApplicationIntent'
        assert _ConnectionStringParser.normalize_key('keepalive') == 'KeepAlive'
        assert _ConnectionStringParser.normalize_key('keepaliveinterval') == 'KeepAliveInterval'
        assert _ConnectionStringParser.normalize_key('ipaddresspreference') == 'IpAddressPreference'
        # Timeout parameters not in restricted allowlist
        assert _ConnectionStringParser.normalize_key('connection timeout') is None
        assert _ConnectionStringParser.normalize_key('login timeout') is None
        assert _ConnectionStringParser.normalize_key('connect timeout') is None
        assert _ConnectionStringParser.normalize_key('timeout') is None
    
    def test_normalize_key_mars(self):
        """Test that MARS parameters are not in the allowlist."""
        assert _ConnectionStringParser.normalize_key('mars_connection') is None
        assert _ConnectionStringParser.normalize_key('mars connection') is None
        assert _ConnectionStringParser.normalize_key('multipleactiveresultsets') is None
    
    def test_normalize_key_app(self):
        """Test normalization of APP parameter."""
        assert _ConnectionStringParser.normalize_key('app') == 'APP'
        assert _ConnectionStringParser.normalize_key('APP') == 'APP'
        # 'application name' is not in restricted allowlist
        assert _ConnectionStringParser.normalize_key('application name') is None
    
    def test_normalize_key_driver(self):
        """Test normalization of Driver parameter."""
        assert _ConnectionStringParser.normalize_key('driver') == 'Driver'
        assert _ConnectionStringParser.normalize_key('DRIVER') == 'Driver'
    
    def test_normalize_key_not_allowed(self):
        """Test normalization of disallowed keys returns None."""
        assert _ConnectionStringParser.normalize_key('BadParam') is None
        assert _ConnectionStringParser.normalize_key('UnsupportedParameter') is None
        assert _ConnectionStringParser.normalize_key('RandomKey') is None
    ...
    def test__normalize_params_empty_dict(self):
        ...
        filtered = _ConnectionStringParser._normalize_params({}, warn_rejected=False)
        assert filtered == {}
