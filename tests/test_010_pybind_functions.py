    ...
    def test_get_driver_path_cpp(self):
        ...
        try:
            ...
            assert any(keyword in error_msg for keyword in [
                "driver not found", "cannot find", "not available", 
...
class TestMemoryManagement:
    ...
    def test_multiple_numeric_data_creation(self):
        ...
        numerics = []
        for i in range(50):
            numeric = ddbc.NumericData(10 + i, 2, 1, f"test_{i}".encode('latin-1').decode('latin-1'))
...
class TestAdditionalPybindFunctions:
    ...
    def test_all_exposed_functions_exist(self):
        ...
        expected_functions = [
            'GetDriverPathCpp', 'ThrowStdException', 'enable_pooling', 'close_pooling',
            'DDBCSetDecimalSeparator', 'DDBCSQLExecDirect', 'DDBCSQLExecute',
            'DDBCSQLRowCount', 'DDBCSQLFetch', 'DDBCSQLNumResultCols', 'DDBCSQLDescribeCol',
            'DDBCSQLGetData', 'DDBCSQLMoreResults', 'DDBCSQLFetchOne', 'DDBCSQLFetchMany',
            'DDBCSQLFetchAll', 'DDBCSQLFreeHandle', 'DDBCSQLCheckError', 'DDBCSQLTables',
            'DDBCSQLFetchScroll', 'DDBCSQLSetStmtAttr', 'DDBCSQLGetTypeInfo'
        ]
    ...
    def test_numeric_data_with_various_inputs(self):
        ...
        for precision, scale, sign, data in test_cases:
            try:
                ...
                assert "length" in str(e).lower() or "size" in str(e).lower() or "runtime" in str(e).lower()
    ...
    def test_connection_pooling_workflow(self):
        ...
        try:
            ...
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in [
                "not supported", "not available", "pooling"
            ])
    ...
    def test_decimal_separator_variations(self):
        ...
        for sep in separators_to_test:
            try:
                ...
                error_msg = str(e).lower()
                assert any(keyword in error_msg for keyword in [
                    "invalid", "not supported", "separator"
                ])
    ...
    def test_driver_path_with_different_drivers(self):
        ...
        for driver_name in driver_names:
            try:
                ...
                error_msg = str(e).lower()
                assert any(keyword in error_msg for keyword in [
                    "not found", "cannot find", "not available", "driver"
                ])
