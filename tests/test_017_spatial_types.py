...
def test_geography_large_polygon_fetch(cursor, db_connection):
    """Test fetching a 100-vertex polygon as bytes."""
    try:
        cursor.execute(
...
def test_geography_description_metadata(cursor, db_connection):
    """Test cursor.description for geography columns maps to bytes."""
    try:
        cursor.execute(
...
def test_geography_binary_output_consistency(cursor, db_connection):
    """Test binary consistency: identical WKT inputs produce identical raw bytes."""
    try:
        cursor.execute(
        ...
        rows = cursor.execute(
            "SELECT geo_col FROM #pytest_geography_binary ORDER BY id;"
        ).fetchall()
        assert len(rows) == 2
...
def test_geometry_description_metadata(cursor, db_connection):
    """Test cursor.description for geometry columns maps to bytes."""
    try:
        cursor.execute(
...
def test_geometry_binary_output_consistency(cursor, db_connection):
    ...
    try:
        ...
        rows = cursor.execute(
            "SELECT geom_col FROM #pytest_geometry_binary ORDER BY id;"
        ).fetchall()
        assert len(rows) == 2
        assert isinstance(rows[0][0], bytes)
        assert isinstance(rows[1][0], bytes)
...
def test_hierarchyid_description_metadata(cursor, db_connection):
    """Test cursor.description for hierarchyid columns maps to bytes."""
    try:
        cursor.execute(
