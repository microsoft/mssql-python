...
def test_cursor_cache_invalidation_different_column_orders(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT CAST('2023-01-01' AS DATE) as date_col, CAST('test' AS VARCHAR(10)) as text_col"
        )
...
def test_cursor_cache_invalidation_metadata_then_select(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT meta_id, meta_name, meta_value, meta_date, meta_flag FROM #test_metadata_table ORDER BY meta_id"
        )
        ...
        cursor.execute(
            "SELECT COUNT(*) as total_count, AVG(meta_value) as avg_value FROM #test_metadata_table"
        )
...
def test_cursor_cache_invalidation_fetch_methods_consistency(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT first_col, second_col, third_col FROM #test_fetch_cache ORDER BY second_col"
        )
        ...
        cursor.execute(
            "SELECT third_col, second_col, first_col FROM #test_fetch_cache ORDER BY second_col"
        )
        ...
        cursor.execute(
            "SELECT second_col, first_col FROM #test_fetch_cache WHERE second_col > 20 ORDER BY second_col"
        )
    ...
    for iteration in range(100):  # Reduced from thousands for practical test execution
        ...
        try:
            ...
            if col_suffix == 0:
                ...
                cursor.execute(
                    f"SELECT 'str_{iteration}' as str_col, {iteration * 2} as num_col, {iteration * 3.14} as float_col"
                )
            elif col_suffix == 2:
                cursor.execute(
                    f"SELECT {iteration} as a, {iteration+1} as b, {iteration+2} as c, {iteration+3} as d"
                )
            else:
                cursor.execute(
                    f"SELECT 'batch_{iteration}' as batch_id, {iteration % 2} as flag_col"
                )
...
def test_error_recovery_cache_state_validation(db_connection):
    ...
    try:
        ...
        try:
            ...
            assert (
                "non_existent_table" in error_msg or "invalid" in error_msg or "object" in error_msg
            )
