...
def test_basic_multithreaded_queries(stress_conn_str, num_threads, iterations):
    ...
    print(
        f"[PASSED] {num_threads} threads × {iterations} iterations = {result.throughput_qps:.1f} qps"
    )
...
def test_basic_multithreaded_without_pooling(stress_conn_str, num_threads, iterations):
    ...
    assert result.success, f"Test failed with {result.total_errors} errors"
    print(
        f"[PASSED] {num_threads} threads × {iterations} iterations (no pooling) = {result.throughput_qps:.1f} qps"
    )
...
def test_medium_load_multithreaded(stress_conn_str, num_threads, iterations):
    ...
    print(
        f"[PASSED] {num_threads} threads × {iterations} iterations = {result.throughput_qps:.1f} qps, {error_rate*100:.1f}% errors"
    )
...
@pytest.mark.stress_threading
def test_mixed_read_write_multithreaded(stress_conn_str):
    """
    Test concurrent read and write operations.

    Half the threads do reads, half do writes to test for deadlocks.
    """
    num_threads = 20
    iterations = 20

    # Setup: create shared table
    setup_conn = connect(stress_conn_str)
    setup_cursor = setup_conn.cursor()
    drop_table_if_exists(setup_cursor, "##stress_rw_test")
    setup_cursor.execute(
        """
        CREATE TABLE ##stress_rw_test (
    ...
    def writer_worker(thread_id: int):
        try:
            ...
            for i in range(iterations):
                cursor.execute(
                    "INSERT INTO ##stress_rw_test (thread_id, value) VALUES (?, ?)",
    ...
    def reader_worker(thread_id: int):
        try:
            ...
            cursor = conn.cursor()
            for i in range(iterations):
                cursor.execute("SELECT TOP 100 * FROM ##stress_rw_test ORDER BY id DESC")
                rows = cursor.fetchall()
                with results_lock:
                    results["reads"] += 1
            cursor.close()
    ...
    # Start threads: half writers, half readers
    threads = []
    for i in range(num_threads // 2):
        threads.append(threading.Thread(target=writer_worker, args=(i,), daemon=True))
        threads.append(threading.Thread(target=reader_worker, args=(i + 100,), daemon=True))
    ...
    # Cleanup
    cleanup_conn = connect(stress_conn_str)
    cleanup_cursor = cleanup_conn.cursor()
    drop_table_if_exists(cleanup_cursor, "##stress_rw_test")
    cleanup_conn.commit()
    cleanup_cursor.close()
    cleanup_conn.close()

    total_ops = results["reads"] + results["writes"]
    expected_ops = num_threads * iterations
    completion_rate = total_ops / expected_ops

    print(f"\nMixed read/write test:")
    print(f"  Reads: {results['reads']}, Writes: {results['writes']}")
    print(f"  Errors: {len(errors)}")
    print(f"  Completion rate: {completion_rate*100:.1f}%")

    assert completion_rate >= 0.9, f"Too many operations failed: {completion_rate*100:.1f}%"
    print(f"[PASSED] Mixed read/write multi-threaded test")
...
def test_comprehensive_thread_scaling(stress_conn_str, num_threads, iterations, pooling):
    ...
    print(
        f"[PASSED] {num_threads}T × {iterations}I, pooling={pooling}: "
