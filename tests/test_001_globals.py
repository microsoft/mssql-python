...
# ---------------------------------------------------------
# GH-609: Unit tests for executemany SQL_C_NUMERIC mismatch
# ---------------------------------------------------------
import decimal as _decimal
from mssql_python.cursor import Cursor, MONEY_MAX
...
def test_compute_column_type_large_decimal():
    ...
    column = [
        ...
        _decimal.Decimal("999999999999999999.123456"),  # > MONEY_MAX
        _decimal.Decimal("200.75"),
    ]
    sample, _, _, max_dec_len = cur._compute_column_type(column)
    assert isinstance(sample, _decimal.Decimal)
    assert sample > MONEY_MAX
...
def test_map_sql_type_decimal_outside_money_returns_numeric():
    ...
    This C type expects NumericData structs, but executemany's conversion
    loop converts Decimals to strings — creating a mismatch that the
    GH-609 override in executemany must correct.
    """
    cur = _make_bare_cursor()
    val = _decimal.Decimal("999999999999999999.123456")
    dummy_row = [val]
    ...
    # The executemany conversion loop would turn this into a string,
    # confirming the override to SQL_C_CHAR is necessary.
...
def test_executemany_numeric_override_needed():
    ...
    Verifies the full type-detection pipeline that executemany uses: when a
    column of large Decimals is detected, _create_parameter_types_list returns
    SQL_C_NUMERIC, but the override in executemany must change it to SQL_C_CHAR
    since the conversion loop converts all Decimals to strings.
    """
    ...
    data = [
        (_decimal.Decimal("100.50"),),
        (_decimal.Decimal("999999999999999999.123456"),),
    ]

    # Step 1: type detection (same as executemany's auto-detection path)
    ...
    # Before the fix: C type would be SQL_C_NUMERIC (mismatch with string data)
    assert paraminfo.paramSQLType == _C.SQL_NUMERIC.value
    original_c_type = paraminfo.paramCType

    # Step 2: apply the GH-609 override (same logic as cursor.py executemany)
    ...
    # After the fix: C type is SQL_C_CHAR, matching the string conversion
    assert paraminfo.paramCType == _C.SQL_C_CHAR.value
    assert original_c_type == _C.SQL_C_NUMERIC.value  # confirms override was needed
...
def test_executemany_decimal_numeric_override_coverage(monkeypatch):
    ...
    Mocks only the I/O boundaries (_check_closed, _reset_cursor, SQLExecuteMany)
    so the entire type-detection and conversion pipeline runs for real.
    """
    ...
    # Capture what gets passed to SQLExecuteMany
    captured = {}
    ...
    # Verify the GH-609 override was applied inside the real executemany
    pt = captured["parameters_type"]
    assert len(pt) == 1
    assert pt[0].paramSQLType == _C.SQL_NUMERIC.value
    assert (
        pt[0].paramCType == _C.SQL_C_CHAR.value
    ), f"Expected SQL_C_CHAR ({_C.SQL_C_CHAR.value}), got {pt[0].paramCType}"

    # Verify values were converted to strings (not NumericData structs)
