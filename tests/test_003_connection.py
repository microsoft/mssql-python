...
def test_setencoding_explicit_ctype_override(db_connection):
    ...
    settings = db_connection.getencoding()
    assert settings['encoding'] == 'utf-16le', "Encoding should be forced to utf-16le for SQL_WCHAR"
...
def test_setdecoding_automatic_ctype_detection(db_connection):
    ...
    for encoding in other_encodings:
        ...
        settings = db_connection.getdecoding(mssql_python.SQL_WCHAR)
        assert settings['encoding'] == 'utf-16le', f"SQL_WCHAR with {encoding} should be forced to utf-16le"
        assert settings['ctype'] == mssql_python.SQL_WCHAR, f"SQL_WCHAR should maintain SQL_WCHAR ctype"
...
def test_setdecoding_explicit_ctype_override(db_connection):
    ...
    settings = db_connection.getdecoding(mssql_python.SQL_CHAR)
    assert settings['encoding'] == 'utf-16le', "Encoding should be forced to utf-16le for SQL_WCHAR ctype"
...
def test_setdecoding_common_encodings(db_connection):
    ...
    for encoding in other_encodings:
        try:
            ...
            db_connection.setdecoding(mssql_python.SQL_WCHAR, encoding=encoding)
            settings = db_connection.getdecoding(mssql_python.SQL_WCHAR)
            assert settings['encoding'] == 'utf-16le', f"SQL_WCHAR should force {encoding} to utf-16le"
        except Exception as e:
...
def test_setdecoding_getdecoding_consistency(db_connection):
    ...
    test_cases = [
        ...
        (mssql_python.SQL_WCHAR, 'latin-1', mssql_python.SQL_WCHAR, 'utf-16le'),  # latin-1 forced to utf-16le
    ...
    for sqltype, input_encoding, expected_ctype, expected_encoding in test_cases:
        ...
        assert settings['encoding'] == expected_encoding.lower(), f"Encoding should be {expected_encoding.lower()}"
...
# ===============================================================
# SQL_WCHAR Encoding Restriction Tests
# ===============================================================

def test_sql_wchar_encoding_restriction_setencoding(db_connection):
    """Test that SQL_WCHAR only accepts UTF-16 encodings in setencoding."""
    
    # Test that UTF-16 encodings work with SQL_WCHAR
    utf16_encodings = ['utf-16', 'utf-16le', 'utf-16be']
    for encoding in utf16_encodings:
        db_connection.setencoding(encoding=encoding, ctype=mssql_python.SQL_WCHAR)
        settings = db_connection.getencoding()
        assert settings['encoding'] == encoding.lower(), f"UTF-16 encoding {encoding} should be accepted"
        assert settings['ctype'] == mssql_python.SQL_WCHAR, f"ctype should remain SQL_WCHAR for {encoding}"

    # Test that non-UTF-16 encodings are forced to UTF-16LE with SQL_WCHAR
    non_utf16_encodings = ['utf-8', 'latin-1', 'ascii', 'cp1252', 'gbk', 'shift_jis']
    for encoding in non_utf16_encodings:
        db_connection.setencoding(encoding=encoding, ctype=mssql_python.SQL_WCHAR)
        settings = db_connection.getencoding()
        assert settings['encoding'] == 'utf-16le', f"Non-UTF-16 encoding {encoding} should be forced to utf-16le"
        assert settings['ctype'] == mssql_python.SQL_WCHAR, f"ctype should remain SQL_WCHAR for {encoding}"

def test_sql_wchar_encoding_restriction_setdecoding(db_connection):
    """Test that SQL_WCHAR only accepts UTF-16 encodings in setdecoding."""
    ...
    for encoding in utf16_encodings:
        ...
        settings = db_connection.getdecoding(mssql_python.SQL_WCHAR)
        assert settings['encoding'] == encoding.lower(), f"UTF-16 encoding {encoding} should be accepted"
        assert settings['ctype'] == mssql_python.SQL_WCHAR, f"ctype should be SQL_WCHAR for {encoding}"
...
def test_sql_wmetadata_encoding_restriction(db_connection):
    """Test that SQL_WMETADATA only accepts UTF-16 encodings."""
    
    # Test that UTF-16 encodings work with SQL_WMETADATA
    utf16_encodings = ['utf-16', 'utf-16le', 'utf-16be']
    for encoding in utf16_encodings:
        db_connection.setdecoding(mssql_python.SQL_WMETADATA, encoding=encoding)
        settings = db_connection.getdecoding(mssql_python.SQL_WMETADATA)
        assert settings['encoding'] == encoding.lower(), f"UTF-16 encoding {encoding} should be accepted for SQL_WMETADATA"

    # Test that non-UTF-16 encodings are forced to UTF-16LE with SQL_WMETADATA
    non_utf16_encodings = ['utf-8', 'latin-1', 'ascii', 'cp1252']
    for encoding in non_utf16_encodings:
        db_connection.setdecoding(mssql_python.SQL_WMETADATA, encoding=encoding)
        settings = db_connection.getdecoding(mssql_python.SQL_WMETADATA)
        assert settings['encoding'] == 'utf-16le', f"Non-UTF-16 encoding {encoding} should be forced to utf-16le for SQL_WMETADATA"

def test_sql_char_encoding_flexibility(db_connection):
    """Test that SQL_CHAR accepts various encodings without restriction."""
    
    # Test various encodings with SQL_CHAR - all should work
    all_encodings = ['utf-8', 'utf-16le', 'utf-16be', 'utf-16', 'latin-1', 'ascii', 'cp1252']
    
    for encoding in all_encodings:
        # Test with setencoding
        db_connection.setencoding(encoding=encoding, ctype=mssql_python.SQL_CHAR)
        settings = db_connection.getencoding()
        assert settings['encoding'] == encoding.lower(), f"SQL_CHAR should accept encoding {encoding} in setencoding"
        assert settings['ctype'] == mssql_python.SQL_CHAR, f"ctype should remain SQL_CHAR for {encoding}"
        
        # Test with setdecoding
        db_connection.setdecoding(mssql_python.SQL_CHAR, encoding=encoding)
        settings = db_connection.getdecoding(mssql_python.SQL_CHAR)
        assert settings['encoding'] == encoding.lower(), f"SQL_CHAR should accept encoding {encoding} in setdecoding"

def test_automatic_ctype_detection_with_restrictions(db_connection):
    """Test automatic ctype detection respects SQL_WCHAR restrictions."""
    
    # Test UTF-16 encodings automatically get SQL_WCHAR
    utf16_encodings = ['utf-16', 'utf-16le', 'utf-16be']
    for encoding in utf16_encodings:
        db_connection.setencoding(encoding=encoding)  # No explicit ctype
        settings = db_connection.getencoding()
        assert settings['ctype'] == mssql_python.SQL_WCHAR, f"UTF-16 encoding {encoding} should auto-detect SQL_WCHAR"
        assert settings['encoding'] == encoding.lower(), f"Encoding should remain {encoding}"

    # Test non-UTF-16 encodings automatically get SQL_CHAR
    non_utf16_encodings = ['utf-8', 'latin-1', 'ascii', 'cp1252']
    for encoding in non_utf16_encodings:
        db_connection.setencoding(encoding=encoding)  # No explicit ctype
        settings = db_connection.getencoding()
        assert settings['ctype'] == mssql_python.SQL_CHAR, f"Non-UTF-16 encoding {encoding} should auto-detect SQL_CHAR"
        assert settings['encoding'] == encoding.lower(), f"Encoding should remain {encoding}"

def test_mixed_encoding_scenarios(db_connection):
    """Test complex scenarios with mixed encoding and ctype combinations."""
    
    # Scenario 1: Set valid UTF-16 with SQL_WCHAR, then try to override with invalid encoding
    db_connection.setencoding(encoding='utf-16le', ctype=mssql_python.SQL_WCHAR)
    settings = db_connection.getencoding()
    assert settings['encoding'] == 'utf-16le'
    assert settings['ctype'] == mssql_python.SQL_WCHAR
    
    # Now try to set invalid encoding with SQL_WCHAR - should be forced to UTF-16LE
    db_connection.setencoding(encoding='utf-8', ctype=mssql_python.SQL_WCHAR)
    settings = db_connection.getencoding()
    assert settings['encoding'] == 'utf-16le'  # Should be forced
    assert settings['ctype'] == mssql_python.SQL_WCHAR
    
    # Scenario 2: Set different encodings for different SQL types
    db_connection.setdecoding(mssql_python.SQL_CHAR, encoding='utf-8')
    db_connection.setdecoding(mssql_python.SQL_WCHAR, encoding='latin-1')  # Should be forced to UTF-16LE
    db_connection.setdecoding(mssql_python.SQL_WMETADATA, encoding='gbk')  # Should be forced to UTF-16LE
    ...
    metadata_settings = db_connection.getdecoding(mssql_python.SQL_WMETADATA)
    
    assert char_settings['encoding'] == 'utf-8'  # Should remain unchanged
    assert wchar_settings['encoding'] == 'utf-16le'  # Should be forced
    assert metadata_settings['encoding'] == 'utf-16le'  # Should be forced

def test_case_insensitive_encoding_restriction(db_connection):
    """Test that encoding restrictions work with different case variations."""
    
    # Test different case variations of UTF-16 encodings
    utf16_variations = ['UTF-16', 'Utf-16le', 'UTF-16BE', 'utf-16Le']
    for encoding in utf16_variations:
        db_connection.setencoding(encoding=encoding, ctype=mssql_python.SQL_WCHAR)
        settings = db_connection.getencoding()
        assert settings['encoding'] in ['utf-16', 'utf-16le', 'utf-16be'], f"Case variation {encoding} should be normalized and accepted"
        assert settings['ctype'] == mssql_python.SQL_WCHAR
    
    # Test different case variations of non-UTF-16 encodings
    non_utf16_variations = ['UTF-8', 'Latin-1', 'ASCII', 'CP1252']
    for encoding in non_utf16_variations:
        db_connection.setencoding(encoding=encoding, ctype=mssql_python.SQL_WCHAR)
        settings = db_connection.getencoding()
        assert settings['encoding'] == 'utf-16le', f"Case variation {encoding} should be forced to utf-16le"
        assert settings['ctype'] == mssql_python.SQL_WCHAR

def test_edge_case_encodings_with_sql_wchar(db_connection):
    """Test edge case encodings with SQL_WCHAR restrictions."""
    
    # Test some edge case encodings
    edge_encodings = [
        'iso-8859-1',    # Alternative name for latin-1
        'us-ascii',      # Alternative name for ascii
        'windows-1252',  # Alternative name for cp1252
        'utf8',          # Alternative name for utf-8
        'unicode',       # Alternative name that might resolve to utf-16
    ]
    
    for encoding in edge_encodings:
        try:
            db_connection.setencoding(encoding=encoding, ctype=mssql_python.SQL_WCHAR)
            settings = db_connection.getencoding()
            
            # Should either be accepted (if it's a UTF-16 variant) or forced to utf-16le
            if encoding.lower() == 'unicode':
                # 'unicode' might be accepted as it can resolve to UTF-16
                assert settings['encoding'] in ['utf-16le', 'unicode'], f"Unicode encoding handling: {settings['encoding']}"
            else:
                # Non-UTF-16 encodings should be forced to utf-16le
                assert settings['encoding'] == 'utf-16le', f"Edge case encoding {encoding} should be forced to utf-16le"
            
            assert settings['ctype'] == mssql_python.SQL_WCHAR
            
        except Exception as e:
            # Some edge case encodings might not be valid, which is acceptable
            pytest.skip(f"Encoding {encoding} not supported: {e}")

def test_encoding_restriction_behavior(db_connection):
    """Test that encodings are properly restricted and forced to UTF-16LE when needed."""
    
    # Test that non-UTF-16 encoding with SQL_WCHAR gets forced to utf-16le
    db_connection.setencoding(encoding='utf-8', ctype=mssql_python.SQL_WCHAR)
    settings = db_connection.getencoding()
    assert settings['encoding'] == 'utf-16le', "Non-UTF-16 encoding should be forced to utf-16le with SQL_WCHAR"
    assert settings['ctype'] == mssql_python.SQL_WCHAR, "ctype should remain SQL_WCHAR"
    
    # Test that non-UTF-16 encoding with SQL_WCHAR in setdecoding gets forced
    db_connection.setdecoding(mssql_python.SQL_WCHAR, encoding='latin-1')
    settings = db_connection.getdecoding(mssql_python.SQL_WCHAR)
    assert settings['encoding'] == 'utf-16le', "Non-UTF-16 encoding should be forced to utf-16le for SQL_WCHAR"
    assert settings['ctype'] == mssql_python.SQL_WCHAR, "ctype should remain SQL_WCHAR"
    
    # Test that explicit ctype=SQL_WCHAR forces encoding restriction even for SQL_CHAR
    db_connection.setdecoding(mssql_python.SQL_CHAR, encoding='ascii', ctype=mssql_python.SQL_WCHAR)
    settings = db_connection.getdecoding(mssql_python.SQL_CHAR)
    assert settings['encoding'] == 'utf-16le', "Non-UTF-16 encoding should be forced to utf-16le when ctype=SQL_WCHAR"
    assert settings['ctype'] == mssql_python.SQL_WCHAR, "ctype should be SQL_WCHAR when explicitly set"

def test_sql_wchar_restriction_with_explicit_ctype_combinations(db_connection):
    """Test all combinations of explicit ctype with various encodings."""
    
    test_combinations = [
        # (encoding, explicit_ctype, expected_encoding, expected_ctype, description)
        ('utf-16le', mssql_python.SQL_WCHAR, 'utf-16le', mssql_python.SQL_WCHAR, "Valid UTF-16 with SQL_WCHAR"),
        ('utf-8', mssql_python.SQL_WCHAR, 'utf-16le', mssql_python.SQL_WCHAR, "Invalid UTF-8 with SQL_WCHAR should be forced"),
        ('latin-1', mssql_python.SQL_WCHAR, 'utf-16le', mssql_python.SQL_WCHAR, "Invalid latin-1 with SQL_WCHAR should be forced"),
        ('utf-16be', mssql_python.SQL_WCHAR, 'utf-16be', mssql_python.SQL_WCHAR, "Valid UTF-16BE with SQL_WCHAR"),
        ('utf-8', mssql_python.SQL_CHAR, 'utf-8', mssql_python.SQL_CHAR, "UTF-8 with SQL_CHAR should work"),
        ('latin-1', mssql_python.SQL_CHAR, 'latin-1', mssql_python.SQL_CHAR, "latin-1 with SQL_CHAR should work"),
    ]
    
    for encoding, ctype, expected_encoding, expected_ctype, description in test_combinations:
        # Test setencoding
        db_connection.setencoding(encoding=encoding, ctype=ctype)
        settings = db_connection.getencoding()
        assert settings['encoding'] == expected_encoding, f"setencoding {description}: expected {expected_encoding}, got {settings['encoding']}"
        assert settings['ctype'] == expected_ctype, f"setencoding {description}: expected ctype {expected_ctype}, got {settings['ctype']}"
        
        # Test setdecoding for SQL_CHAR and SQL_WCHAR
        for sqltype in [mssql_python.SQL_CHAR, mssql_python.SQL_WCHAR]:
            # Determine expected encoding based on sqltype OR ctype being SQL_WCHAR
            if (sqltype == mssql_python.SQL_WCHAR or ctype == mssql_python.SQL_WCHAR) and encoding not in ['utf-16', 'utf-16le', 'utf-16be']:
                expected_dec_encoding = 'utf-16le'  # Non-UTF-16 should be forced when SQL_WCHAR is involved
            else:
                expected_dec_encoding = encoding.lower()
            
            db_connection.setdecoding(sqltype, encoding=encoding, ctype=ctype)
            settings = db_connection.getdecoding(sqltype)
            
            sqltype_name = "SQL_WCHAR" if sqltype == mssql_python.SQL_WCHAR else "SQL_CHAR"
            assert settings['encoding'] == expected_dec_encoding, f"setdecoding {sqltype_name} {description}: expected {expected_dec_encoding}, got {settings['encoding']}"