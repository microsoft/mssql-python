...
def test_autocommit_setter(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_test_autocommit (id, value) VALUES (1, 'test');"
        )
...
def test_rollback_on_close(conn_str, db_connection):
    ...
    try:
        ...
        assert (
            result is not None
        ), "Rollback on close failed: No data found before close"
        assert (
            result[1] == "test"
        ), "Rollback on close failed: Incorrect data before close"
...
def test_rollback(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_test_rollback (id, value) VALUES (1, 'test');"
        )
        db_connection.commit()
...
def test_close_with_autocommit_true(conn_str):
    ...
    try:
        ...
        verify_cursor.execute(
            "SELECT * FROM pytest_autocommit_close_test WHERE id = 1;"
        )
        result = verify_cursor.fetchone()
        ...
        assert (
            result is not None
        ), "Autocommit failed: Data not found after connection close"
        assert (
...
def test_connection_exception_attributes_exist(db_connection):
    ...
    ), "Connection should have DatabaseError attribute"
    assert hasattr(
        db_connection, "DataError"
    ), "Connection should have DataError attribute"
    assert hasattr(
    ...
    assert hasattr(
        db_connection, "InternalError"
    ), "Connection should have InternalError attribute"
    assert hasattr(
...
def test_connection_exception_attributes_are_classes(db_connection):
    ...
    assert (
        db_connection.Warning is Warning
    ), "Connection.Warning should be the Warning class"
...
def test_connection_exception_instantiation(db_connection):
    ...
    assert isinstance(
        warning, db_connection.Warning
    ), "Should be able to create Warning instance"
...
def test_connection_exception_catching_with_connection_attributes(db_connection):
    ...
    try:
        ...
        pytest.fail(
            f"Should have caught InterfaceError, but got {type(e).__name__}: {e}"
        )
...
def test_connection_exception_error_handling_example(db_connection):
    ...
    try:
        ...
        pytest.fail(
            f"Expected ProgrammingError or DatabaseError, got {type(e).__name__}: {e}"
        )
...
def test_connection_exception_attributes_consistency(conn_str):
    ...
    try:
        ...
        assert (
            conn1.Error is conn2.Error
        ), "All connections should reference the same Error class"
        assert (
        ...
        assert (
            conn1.Error is Error
        ), "Connection.Error should be the same as module-level Error"
        assert (
...
def test_connection_exception_attributes_comprehensive_list():
    ...
    for exc_name in required_exceptions:
        assert hasattr(
            Connection, exc_name
        ), f"Connection class should have {exc_name} attribute"
...
def test_connection_execute(db_connection):
    ...
    if not db_connection.autocommit:
        ...
        assert (
            result[1] == "test_value"
        ), "Execute with table creation returned wrong value"
...
def test_connection_execute_empty_result(db_connection):
    ...
    cursor = db_connection.execute(
        "SELECT * FROM sys.tables WHERE name = 'nonexistent_table_name'"
    )
    result = cursor.fetchone()
...
def test_execute_after_connection_close(conn_str):
    ...
    assert (
        "closed" in str(excinfo.value).lower()
...
def test_execute_with_large_parameters(db_connection):
    ...
    try:
        ...
        # Large batch insert completed successfully
        assert batch_time > 0  # Ensure operation took some time
        ...
        # Large parameter insert completed successfully
        assert large_param_time > 0  # Ensure operation took some time
        ...
        cursor = db_connection.execute(
            "SELECT id, large_text FROM #large_params_test ORDER BY id"
        )
        rows = cursor.fetchall()
        cursor.close()
        ...
        # Large result set fetched successfully
        assert result_time > 0  # Ensure operation took some time
...
def test_connection_execute_cursor_lifecycle(db_connection):
    ...
    assert (
        cursor_ref() is None
    ), "Cursor should be garbage collected after going out of scope"
    assert (
        len(db_connection._cursors) == initial_cursor_count
...
def test_batch_execute_basic(db_connection):
    ...
    assert isinstance(
        results[2][0][0], (str, datetime)
    ), "Third result should be a date"
...
def test_batch_execute_with_parameters(db_connection):
    ...
    assert (
        abs(results[1][0][0] - 3.14159) < 0.00001
    ), "Float parameter not handled correctly"
    assert results[2][0][0] == "test string", "String parameter not handled correctly"
    assert results[3][0][0] == bytearray(
        b"binary data"
    ), "Binary parameter not handled correctly"
...
def test_batch_execute_reuse_cursor(db_connection):
    ...
    results, returned_cursor = db_connection.batch_execute(
        statements, reuse_cursor=cursor
    )
    ...
    assert (
        final_result[0][0] == "after batch"
    ), "Cursor should remain usable after batch"
...
def test_batch_execute_transaction(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE ##batch_transaction_test (id INT, value VARCHAR(50))"
        )
...
def test_batch_execute_error_handling(db_connection):
    ...
    assert (
        "nonexistent_table" in str(excinfo.value).lower()
    ...
    assert (
        new_cursor.fetchone()[0] == 1
    ), "Connection should be usable after batch error"
    new_cursor.close()
...
def test_output_converter_exception_handling(db_connection):
    ...
    try:
        ...
        assert (
            row[0] is not None
        ), "Column value shouldn't be None despite converter error"
        ...
        error_str = str(e)
        assert (
            "sensitive data" not in error_str
        ), f"Exception leaked sensitive data: {error_str}"
        assert not isinstance(
            e, ValueError
        ), "Original exception type should not be exposed"

        # Verify we can continue using the connection after the error
        cursor.execute("SELECT 1 AS test")
        assert (
            cursor.fetchone()[0] == 1
        ), "Connection should still be usable after converter error"
...
def test_timeout_default(db_connection):
    ...
    ), "Connection should have a timeout attribute"
...
def test_getinfo_numeric_limits(db_connection):
    ...
    try:
        ...
        max_col_name_len = db_connection.getinfo(
        ...
        max_table_name_len = db_connection.getinfo(
            sql_const.SQL_MAX_TABLE_NAME_LEN.value
        )
        assert isinstance(
            max_table_name_len, int
        ), "Max table name length should be an integer"
        ...
        ), "Max statement length should be an integer"
        assert max_statement_len >= 0, "Max statement length should be non-negative"

        # Max connections - may return 0 for "unlimited"
        max_connections = db_connection.getinfo(
            sql_const.SQL_MAX_DRIVER_CONNECTIONS.value
        )
...
def test_getinfo_catalog_support(db_connection):
    ...
    try:
        ...
        catalog_separator = db_connection.getinfo(
            sql_const.SQL_CATALOG_NAME_SEPARATOR.value
        )
...
def test_getinfo_transaction_support(db_connection):
    ...
    try:
        ...
        default_txn_isolation = db_connection.getinfo(
            sql_const.SQL_DEFAULT_TXN_ISOLATION.value
        )
        print("Default Transaction isolation = ", default_txn_isolation)
        assert (
            default_txn_isolation is not None
        ), "Default transaction isolation should not be None"
        ...
        assert (
            multiple_txn is not None
        ), "Multiple active transactions support should not be None"
...
def test_getinfo_data_types(db_connection):
    ...
    try:
        ...
        assert isinstance(
            numeric_functions, int
        ), "Numeric functions should be an integer"

        # String functions
        string_functions = db_connection.getinfo(sql_const.SQL_STRING_FUNCTIONS.value)
        assert isinstance(
            string_functions, int
        ), "String functions should be an integer"

        # Date/time functions
        datetime_functions = db_connection.getinfo(
            sql_const.SQL_DATETIME_FUNCTIONS.value
        )
        assert isinstance(
            datetime_functions, int
        ), "Datetime functions should be an integer"
...
def test_getinfo_invalid_info_type(db_connection):
    ...
    assert (
        result is None
    ), f"getinfo should return None for negative info type {negative_type}"
...
def test_getinfo_type_consistency(db_connection):
    ...
    for info_type in info_types:
        ...
        assert type(result1) == type(
            result2
        ), f"Type inconsistency for info type {info_type}"
    ...
    for info_type, expected_type in info_types.items():
        try:
            ...
            if expected_type == str:
                assert isinstance(
                    info_value, str
                ), f"Info type {info_type} should return a string"
...
def test_getinfo_zero_length_return(db_connection):
    ...
    assert isinstance(
        special_chars, str
    ), "Special characters should be returned as a string"
...
def test_connection_searchescape_with_percent(db_connection):
    ...
    try:
        ...
        if results:
            assert (
                "abc%def" in results[0][1]
            ), "Escaped LIKE query did not match correct row"
...
def test_connection_searchescape_with_underscore(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #test_escape_underscore (id INT, text VARCHAR(50))"
        )
        ...
        cursor.execute(
            "INSERT INTO #test_escape_underscore VALUES (3, 'abcdef')"
        )  # No match
...
def test_connection_searchescape_multiple_escapes(db_connection):
    ...
    try:
        ...
        if len(results) == 1:
            assert (
                "abc%def_ghi" in results[0][1]
            ), "Multiple escapes query matched incorrect row"
...
def test_connection_searchescape_consistency(db_connection):
    ...
    if "conn_str" in globals():
        try:
            ...
            assert (
                new_escape == escape1
            ), "Searchescape should be consistent across connections"
            new_conn.close()
        except Exception as e:
...
def test_set_attr_constants_access():
    ...
    for const_name in dm_attr_constants + dm_value_constants:
        assert not hasattr(
            mssql_python, const_name
        ), f"{const_name} should NOT be public API"
    ...
    for invalid_value in invalid_values:
        with pytest.raises(ProgrammingError) as exc_info:
            db_connection.set_attr(
                mssql_python.SQL_ATTR_CONNECTION_TIMEOUT, invalid_value
            )
...
def test_set_attr_invalid_attribute_id(db_connection):
    ...
    try:
        ...
        pytest.fail(
            f"Unexpected exception type for invalid attribute: {type(e).__name__}: {e}"
        )
    ...
    for attr_id, value in attribute_value_pairs:
        try:
            ...
            if not any(
                ...
                pytest.fail(
                    f"Unexpected error setting attribute {attr_id} to {value}: {e}"
                )
    ...
    for attr_id, value in edge_cases:
        try:
            ...
            if "out of range" in str(e).lower():
                ...
                "not supported" not in str(e).lower()
...
def test_set_attr_txn_isolation_effect(db_connection):
    ...
    try:
        ...
        try:
            ...
            cursor1.execute("BEGIN TRANSACTION")
            cursor1.execute(
                "UPDATE ##test_isolation SET value = 'dirty read' WHERE id = 1"
            )
            ...
            if "not supported" not in str(e).lower():
                ...
                pytest.skip(
                    "Transaction isolation level changes not supported by driver"
                )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if elapsed >= 4.5:
                    pytest.skip(
                        "Connection timeout attribute not effective with this driver"
                    )
                ...
                if (
                    ...
                    pytest.skip(
                        f"Connection timeout test encountered non-timeout error: {exc}"
                    )
...
def test_set_attr_packet_size_effect(conn_str):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE ##test_packet_size (id INT, large_data NVARCHAR(MAX))"
        )

        # Insert a very large string
        large_string = "X" * (packet_size // 2)  # Unicode chars take 2 bytes each
        cursor.execute(
            "INSERT INTO ##test_packet_size VALUES (?, ?)", (1, large_string)
        )
        conn.commit()
...
def test_set_attr_current_catalog_effect(db_connection, conn_str):
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sys.databases WHERE database_id > 4 AND name != DB_NAME()"
        )
        rows = cursor.fetchall()
        if not rows:
        ...
        try:
            ...
            assert (
                new_db == other_db
            ), f"Database should have changed to {other_db} but is {new_db}"
...
def test_attrs_before_packet_size(conn_str):
    ...
    conn = connect(
        conn_str, attrs_before={ConstantsDDBC.SQL_ATTR_PACKET_SIZE.value: packet_size}
    )
...
def test_attrs_before_after_only_attributes(conn_str):
    ...
    conn = connect(
        conn_str, attrs_before={ConstantsDDBC.SQL_ATTR_CONNECTION_DEAD.value: 0}
    )
...
def test_set_attr_with_string_attributes_real():
    ...
    try:
        ...
        conn = connect(
            conn_str_base, attrs_before={1006: "tempdb"}
        )  # SQL_ATTR_CURRENT_CATALOG
        conn.close()
    except Exception:
...
def test_set_attr_with_binary_attributes_real():
    ...
    try:
        ...
        conn = connect(
            conn_str_base, attrs_before={1045: binary_value}
        ...
        for encoding, ctype in encoding_configs:
            ...
            for test_name, test_string in test_cases:
                try:
                    ...
                    print(f"✓ {test_name} passed with {encoding}")
                    
                except Exception as e:
                    # Log encoding issues but don't fail the test - this is exploratory
                    print(f"⚠ {test_name} had issues with {encoding}: {e}")
...
def test_encoding_decoding_error_scenarios(db_connection):
    ...
    for invalid_encoding in invalid_encodings:
        try:
            ...
            print(f"✓ {invalid_encoding} correctly raised exception: {type(e).__name__}")
            
        try:
            db_connection.setdecoding(SQL_CHAR, encoding=invalid_encoding)
            print(f"Warning: {invalid_encoding} was accepted by setdecoding")
        except Exception as e:
            print(f"✓ {invalid_encoding} correctly raised exception in setdecoding: {type(e).__name__}")
    ...
    try:
        ...
        db_connection.setdecoding(SQL_WCHAR, encoding="utf-16le", ctype=SQL_WCHAR)
        print("✓ Basic encoding/decoding configuration works")
    except Exception as e:
    ...
    try:
        ...
        print("✓ Mixed encoding settings work")
    except Exception as e:
        print(f"⚠ Mixed encoding settings failed: {e}")
    ...
    try:
        ...
        for test_name, test_data in boundary_test_cases:
            try:
                ...
                print(f"✓ Boundary case {test_name} passed")
...
def test_encoding_decoding_concurrent_settings(db_connection):
    ...
    try:
        ...
        print("✓ Concurrent cursor operations with encoding changes passed")
        ...
        for encoding, ctype in encoding_configs:
            ...
            for test_name, params in param_test_cases:
                try:
                    ...
                    print(f"✓ Parameter binding {test_name} with {encoding} passed")
...
def test_encoding_decoding_sql_wchar_error_enforcement(conn_str):
    ...
    try:
        ...
        print("✓ SQL_WCHAR restriction enforcement passed")
...
def test_encoding_decoding_large_dataset_performance(db_connection):
    ...
    try:
        ...
        for encoding, ctype, desc in configs:
            ...
            print(f"✓ {desc} - Insert: {insert_time:.2f}s, Fetch: {fetch_time:.2f}s")
...
def test_encoding_decoding_connection_isolation(conn_str):
    ...
    try:
        ...
        print("✓ Connection isolation test passed")
...
def test_encoding_decoding_sql_wchar_explicit_error_validation(db_connection):
    ...
    print("✓ SQL_WCHAR explicit validation passed")
...
def test_encoding_decoding_metadata_columns(db_connection):
    ...
    try:
        ...
        print("✓ Metadata column name encoding test passed")
        
    except Exception as e:
        # Some SQL Server versions might not support Unicode in column names
        if "identifier" in str(e).lower() or "invalid" in str(e).lower():
            print("⚠ Unicode column names not supported in this SQL Server version, skipping")
        else:
        ...
        for encoding, ctype, config_name in encoding_configs:
            ...
            for dataset in test_datasets:
                try:
                    ...
                    print(f"⚠ Insert failed for dataset with {config_name}: {e}")
            ...
            print(f"✓ Stress test with {config_name} completed successfully")
            
        print("✓ Comprehensive encoding stress test passed")
    ...
    try:
        ...
        for encoding_test in encoding_tests:
            ...
            try:
                ...
                print(f"  ✗ Failed to set encoding {encoding}: {e}")
                results_summary.append({
                    "encoding": encoding_name,
                    "encoding_key": encoding,
                    "total_tests": 0,
        ...
        print("✓ SQL_CHAR encoding variety test completed")
...
def test_encoding_decoding_sql_char_with_unicode_fallback(db_connection):
    ...
    try:
        ...
        print("\n✓ Unicode fallback behavior test completed")
...
def test_encoding_decoding_sql_char_native_character_sets(db_connection):
    ...
    try:
                    ...
                    try:
                        ...
                        if result:
                            ...
                            if retrieved_data == test_data and retrieved_encoding == encoding:
                                print(f"  ✓ {test_name:12} | '{test_data}' → '{retrieved_data}' (Perfect match)")
                                results.append("PASS")
                            else:
                                print(f"  ⚠ {test_name:12} | '{test_data}' → '{retrieved_data}' (Data changed)")
                                results.append("CHANGED")
                        else:
                            print(f"  ✗ {test_name:12} | No data retrieved")
                            results.append("FAIL")
                            
                    except Exception as e:
                        print(f"  ✗ {test_name:12} | Error: {str(e)[:40]}...")
        ...
        print(f"\n{'='*70}")
        print("✓ Native character set testing completed")
                    ...
                    try:
                        ...
                        if result:
                            ...
                            if retrieved == test_data:
                                print(f"  ✓ {test_name:15} | Length: {data_length:3} | Perfect preservation")
                            else:
                                print(f"  ⚠ {test_name:15} | Length: {data_length:3} → {retrieved_length:3} | Data modified")
                        ...
                        print(f"  ✗ {test_name:15} | Error: {str(e)[:30]}...")
                
            except Exception as e:
                print(f"  ✗ Failed to configure {encoding}: {e}")
        
        print(f"\n{'='*60}")
        print("✓ Boundary encoding testing completed")
...
def test_encoding_decoding_sql_char_unicode_issue_diagnosis(db_connection):
    ...
    try:
        ...
        print("✓ Unicode issue diagnosis completed")
...
def test_encoding_decoding_sql_char_best_practices_guide(db_connection):
    ...
    try:
        ...
        test_cases = [
            {
                ...
                "recommendation": "✓ Safe for both VARCHAR and NVARCHAR"
            },
            {
                ...
                "recommendation": "✓ Use NVARCHAR for original, VARCHAR for ASCII version"
            },
            {
                ...
                "recommendation": "✓ NVARCHAR required for Chinese characters"
            },
            {
                ...
                "recommendation": "✓ NVARCHAR essential for mixed scripts and emojis"
            }
        ]
        ...
        if result:
            print(f"✓ Unicode Name (NVARCHAR): {result[0]}")
            print(f"✓ Unicode Description (NVARCHAR): {result[1]}")
            print("✓ Perfect Unicode preservation using NVARCHAR columns!")
