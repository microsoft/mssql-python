...
class TestConnectAPIIntegration:
    ...
    def test_connect_kwargs_override_connection_string(self, mock_ddbc_conn):
        ...
        conn = connect(
            "Server=original;Database=originaldb", Server="overridden", Database="overriddendb"
        )
        ...
        assert (
            "original" not in conn.connection_str.lower()
            or "originaldb" not in conn.connection_str.lower()
        )
    ...
    @pytest.mark.skipif(
        not os.getenv("DB_CONNECTION_STRING"), reason="Requires database connection string"
    )
    ...
    def test_app_name_received_by_sql_server(self, conn_str):
        ...
        with connect(conn_str) as conn:
            ...
            assert (
                app_name_received == "MSSQL-Python"
