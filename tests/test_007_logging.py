    ...
    def test_thread_id_consistent_in_same_thread(self, cleanup_logger):
        ...
        for line in lines:
            if "Message" in line and not line.startswith("#"):  # Skip header and metadata
                parts = [p.strip() for p in line.split(",")]
                if (
                    len(parts) >= 6 and parts[1].isdigit()
                ):  # Ensure it's a data row with numeric thread ID
...
class TestThreadSafety:
        ...
        def reconfigure_repeatedly():
            ...
            try:
                for i in range(10):
                    ...
                    setup_logging(output=mode, log_file_path=log_file if mode == FILE else None)
                    time.sleep(0.005)
            except Exception as e:
    ...
    @pytest.mark.skip(
        reason="Flaky on LocalDB/slower systems - TODO: Increase timing tolerance or skip on CI"
    )
        ...
        def reconfigure_aggressively():
            ...
            try:
                ...
                for i in range(30):
                    ...
                    setup_logging(
                        output=mode, log_file_path=log_file if mode in (FILE, BOTH) else None
                    )
                    reconfig_count[0] += 1
...
class TestExceptionSafety:
    ...
    def test_disk_full_simulation(self, cleanup_logger, temp_log_dir):
        ...
        with mock.patch.object(
            logger._logger, "log", side_effect=OSError("No space left on device")
        ):
            # Should not crash
    ...
    def test_permission_denied_simulation(self, cleanup_logger, temp_log_dir):
        ...
        with mock.patch.object(
            logger._logger, "log", side_effect=PermissionError("Permission denied")
        ):
            # Should not crash
