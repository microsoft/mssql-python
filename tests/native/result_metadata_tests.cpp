...
static void TestChildCallbacks() {
    std::mutex childMutex;
    auto first = std::make_shared<TestHandle>();
    auto second = std::make_shared<TestHandle>();
    std::vector<std::weak_ptr<TestHandle>> children{first, {}, second};
    Populate(first->resultMetadata);
    Populate(second->resultMetadata);
    const auto secondBefore = second->resultMetadata.snapshot();
    int calls = 0;
    ClearChildResultMetadata(childMutex, children, [&](const auto& handle) {
        assert(!handle->resultMetadata.snapshot().metadata);
        if (calls == 0) {
            assert(handle == first);
            assert(second->resultMetadata.snapshot().metadata == secondBefore.metadata);
        } else {
            assert(calls == 1 && handle == second);
            assert(!first->resultMetadata.snapshot().metadata);
        }
        ++calls;
    });
...
static void TestChildCallbackFailure() {
    std::mutex childMutex;
    auto first = std::make_shared<TestHandle>();
    auto second = std::make_shared<TestHandle>();
    first->childMutex = &childMutex;
    const std::weak_ptr<TestHandle> weak = first;
    std::vector<std::weak_ptr<TestHandle>> children{first, second};
    Populate(first->resultMetadata);
    Populate(second->resultMetadata);
    const auto secondBefore = second->resultMetadata.snapshot();
    ...
    try {
        ClearChildResultMetadata(childMutex, children, [&](const auto& handle) {
            ++calls;
            assert(handle == first && !handle->resultMetadata.snapshot().metadata);
            assert(second->resultMetadata.snapshot().metadata == secondBefore.metadata);
            first.reset();
            throw std::runtime_error("first child cleanup failure");
        });
    } catch (const std::runtime_error& error) {
        caught = true;
        assert(std::strcmp(error.what(), "first child cleanup failure") == 0);
    }
    assert(caught && calls == 1 && weak.expired());
...
int main(int argc, char** argv) {
    ...
    if (std::strcmp(name, "snapshots") == 0) {
        ...
        TestLastOwner();
    } else if (std::strcmp(name, "child_callbacks") == 0) {
        TestChildCallbacks();
    } else if (std::strcmp(name, "child_callback_failure") == 0) {
        TestChildCallbackFailure();
    } else {
