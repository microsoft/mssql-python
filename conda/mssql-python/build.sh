...
if "$PYTHON" -c "import sys" >/dev/null 2>&1; then
  ...
  # macOS universal2 wheels are cpXY-specific (they carry a compiled ddbc_bindings),
  # so filter on the target CONDA_PY -- as bld.bat does for the win_arm64 slice -- to
  # never silently grab another interpreter's wheel if WHEELS_DIR ever stages more
  # than one. The pipeline stages exactly one today; this is defense-in-depth.
  code_whl=""
