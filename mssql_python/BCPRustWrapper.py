...
class BCPRustWrapper:
    ...
    def __del__(self):
        ...
        try:
            if self._rust_connection is not None:
                logger.warning("BCPRustWrapper connection was not explicitly closed, cleaning up in destructor")
                self.close()
        except Exception:
    ...
    def connect(self, connection_string: Optional[str] = None):
        ...
        try:
            ...
            python_client_context = {
                ...
                "trust_server_certificate": params.get("trustservercertificate", "yes").lower() in ["yes", "true"],
                "encryption": "Optional",
            }
            
            logger.info("Attempting to connect to server: %s, database: %s", 
                       python_client_context["server"], python_client_context["database"])
    ...
    def bulkcopy(self, table_name: str, data: Iterable, batch_size: int = 1000, 
                 timeout: int = 30, column_mappings: Optional[List[Tuple[int, str]]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                "Invalid cursor or method call for table '%s': %s",
                table_name, str(ae)
            )
            raise RuntimeError(f"Bulk copy method error: {str(ae)}") from ae
        except TypeError as te:
            logger.error(
                "Invalid data type or parameters for table '%s': %s",
                table_name, str(te)
            )
            raise ValueError(f"Invalid bulk copy parameters: {str(te)}") from te
        except Exception as e:
            logger.error(
                "Bulk copy failed for table '%s': %s - %s",
                table_name, type(e).__name__, str(e)
            )
            raise
        finally:
