...
UTF16_ENCODINGS = frozenset(["utf-16", "utf-16le", "utf-16be"])
...
class Connection:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # When an object is only weakly referenced, it can be
        # garbage collected even if it's still in the set.
        # It prevents memory leaks by ensuring that cursors are
        # cleaned up when no longer in use without requiring explicit deletion.
    ...
    def setencoding(self, encoding: str = None, ctype: int = None) -> None:
        ...
        Args:
            ...
            ctype (int, optional): The C data type to use when passing data:
                ...
                UTF-16 variants (see UTF16_ENCODINGS constant). 
                SQL_CHAR is used for all other encodings.
    ...
    def setdecoding(self, sqltype: int, encoding: str = None, ctype: int = None) -> None:
        ...
        Example:
            ...
            # Use explicit ctype
            cnxn.setdecoding(
                mssql_python.SQL_WCHAR, encoding='utf-16le',
        ...
        if sqltype not in valid_sqltypes:
            ...
            raise ProgrammingError(
                ...
                ddbc_error=(
                    ...
                    f"SQL_WCHAR ({ConstantsDDBC.SQL_WCHAR.value}), "
                    f"or SQL_WMETADATA ({SQL_WMETADATA})"
                ),
            )
        ...
        if ctype not in valid_ctypes:
            ...
            raise ProgrammingError(
                driver_error=f"Invalid ctype: {ctype}",
                ddbc_error=(
                    f"ctype must be SQL_CHAR ({ConstantsDDBC.SQL_CHAR.value}) "
                    f"or SQL_WCHAR ({ConstantsDDBC.SQL_WCHAR.value})"
                ),
            )
            ...
            for cursor in cursors_to_close:
                try:
                    if not cursor.closed:
                        cursor.close()
                except (InterfaceError, ProgrammingError, DatabaseError) as e:
        ...
        try:
            if self._conn:
                if not self.autocommit:
                    ...
                    # For autocommit True, this is not necessary as each
