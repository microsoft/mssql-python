...
class AADAuth:
    ...
    def get_token_struct(token: str) -> bytes:
        ...
        logger.debug('get_token_struct: Converting token to SQL Server format - token_length=%d chars', len(token))
    ...
    def get_token(auth_type: str) -> bytes:
        ...
        logger.info('get_token: Starting Azure AD authentication - auth_type=%s, credential_class=%s', 
                    auth_type, credential_class.__name__)
        ...
        try:
            ...
            logger.debug('get_token: Requesting token from Azure AD - scope=https://database.windows.net/.default')
            token = credential.get_token("https://database.windows.net/.default").token
            logger.info('get_token: Azure AD token acquired successfully - token_length=%d chars', len(token))
            return AADAuth.get_token_struct(token)
        except ClientAuthenticationError as e:
            # Re-raise with more specific context about Azure AD authentication failure
            logger.error('get_token: Azure AD authentication failed - credential_class=%s, error=%s', 
                        credential_class.__name__, str(e))
            raise RuntimeError(
            ...
            logger.error('get_token: Unexpected error during credential creation - credential_class=%s, error=%s', 
                        credential_class.__name__, str(e))
            raise RuntimeError(
...
def process_auth_parameters(parameters: List[str]) -> Tuple[List[str], Optional[str]]:
    ...
    logger.debug('process_auth_parameters: Processing complete - auth_type=%s, param_count=%d', 
                 auth_type, len(modified_parameters))
...
def remove_sensitive_params(parameters: List[str]) -> List[str]:
    ...
    logger.debug('remove_sensitive_params: Removing sensitive parameters - input_count=%d', len(parameters))
    exclude_keys = [
        "uid=",
        "pwd=",
        "encrypt=",
    ...
    logger.debug('remove_sensitive_params: Sensitive parameters removed - output_count=%d', len(result))
    return result
...
def get_auth_token(auth_type: str) -> Optional[bytes]:
    ...
    try:
        ...
        logger.warning('get_auth_token: Token acquisition failed - auth_type=%s, error=%s', auth_type, str(e))
        return None
...
def process_connection_string(
    ...
    """
    logger.debug('process_connection_string: Starting - conn_str_length=%d', len(connection_string) if isinstance(connection_string, str) else 0)
    # Check type first
    if not isinstance(connection_string, str):
        logger.error('process_connection_string: Invalid type - expected str, got %s', type(connection_string).__name__)
    ...
    logger.debug('process_connection_string: Split connection string - parameter_count=%d', len(parameters))

    # Validate that there's at least one valid parameter
    if not any("=" in param for param in parameters):
        logger.error('process_connection_string: Invalid connection string format - no key=value pairs found')
        raise ValueError("Invalid connection string format")

    modified_parameters, auth_type = process_auth_parameters(parameters)

    if auth_type:
        logger.info('process_connection_string: Authentication type detected - auth_type=%s', auth_type)
        modified_parameters = remove_sensitive_params(modified_parameters)
        token_struct = get_auth_token(auth_type)
        if token_struct:
            logger.info('process_connection_string: Token authentication configured successfully - auth_type=%s', auth_type)
            return ";".join(modified_parameters) + ";", {1256: token_struct}
        else:
            logger.warning('process_connection_string: Token acquisition failed, proceeding without token')

    logger.debug('process_connection_string: Connection string processing complete - has_auth=%s', bool(auth_type))
