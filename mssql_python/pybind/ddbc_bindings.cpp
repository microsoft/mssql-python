...
// Struct to hold parameter information for binding.
...
std::string MakeParamMismatchErrorStr(
    const SQLSMALLINT cType, const int paramIndex) {
...
// This function allocates a buffer of ParamType,
// stores it as a void* in paramBuffers for
// book-keeping and then returns a ParamType*
// to the allocated memory.
// ctorArgs are the arguments to ParamType's constructor
// used while creating/allocating ParamType
template <typename ParamType, typename... CtorArgs>
ParamType* AllocateParamBuffer(
...
ParamType* AllocateParamBufferArray(
            std::vector<std::shared_ptr<void>>& paramBuffers,
            size_t count) {
    std::shared_ptr<ParamType> buffer(
        new ParamType[count],
        std::default_delete<ParamType[]>());
...
// Given a list of parameters and their ParamInfo,
// calls SQLBindParameter on each of them with
...
SQLRETURN BindParameters(SQLHANDLE hStmt, const py::list& params,
    ...
    for (int paramIndex = 0; paramIndex < params.size(); paramIndex++) {
        ...
        LOG("Binding parameter {} - C Type: {}, SQL Type: {}",
        ...
        switch (paramInfo.paramCType) {
                    ...
                    dataPtr =
                        const_cast<void*>(
                            reinterpret_cast<const void*>(
                                &paramInfos[paramIndex]));
                    strLenOrIndPtr = AllocateParamBuffer<SQLLEN>(paramBuffers);
                    *strLenOrIndPtr = SQL_LEN_DATA_AT_EXEC(0);
                    bufferLength = 0;
                } else {
                    std::string* strParam =
                        AllocateParamBuffer<std::string>(
                            paramBuffers,
                            param.cast<std::string>());
            ...
            case SQL_C_BINARY: {
                ...
                if (strParam->size() > 8192 /* TODO: Fix max length */) {
                    ThrowStdException(
                        "Streaming parameters is not yet supported. "
                        "Parameter size must be less than 8192 bytes");
                }
                dataPtr =
                    const_cast<void*>(
                        static_cast<const void*>(
            ...
            case SQL_C_WCHAR: {
                ...
                if (paramInfo.isDAE) {
                    // deferred execution
                    LOG("Parameter[{}] is marked for"
                        " DAE streaming", paramIndex);
                    dataPtr =
                    const_cast<void*>(
                        reinterpret_cast<const void*>(
                            &paramInfos[paramIndex]));
                    ...
                    *strLenOrIndPtr = SQL_LEN_DATA_AT_EXEC(0);
                    ...
                    LOG(
                        "SQL_C_WCHAR Parameter[{}]: Length={}, isDAE={}",
            ...
            case SQL_C_DEFAULT: {
                ...
                if (sqlType == SQL_UNKNOWN_TYPE) {
                    ...
                    if (!SQL_SUCCEEDED(rc)) {
                        LOG(
                            "SQLDescribeParam failed for parameter "
                            "{} with error code {}",
                            paramIndex, rc);
                        return rc;
                    }
            ...
            case SQL_C_USHORT: {
                ...
                if (value > std::numeric_limits<uint16_t>::max()) {
                    ThrowStdException(
                        "Unsigned short integer parameter out"
                        " of range at paramIndex " +
                        std::to_string(paramIndex));
                }
                dataPtr = static_cast<void*>(
                    AllocateParamBuffer<unsigned int>(
                        paramBuffers,
                        param.cast<unsigned int>()));
                break;
            }
            case SQL_C_SBIGINT:
            case SQL_C_SLONG:
            case SQL_C_LONG: {
                ...
                if (value < std::numeric_limits<int64_t>::min() ||
                    ...
                    ThrowStdException(
                        "Signed 64-bit integer parameter out"
                        " of range at paramIndex " +
                        std::to_string(paramIndex));
                }
                dataPtr = static_cast<void*>(
                    AllocateParamBuffer<int64_t>(
                        paramBuffers,
                        param.cast<int64_t>()));
                break;
            }
            ...
            case SQL_C_ULONG: {
                ...
                if (value > std::numeric_limits<uint64_t>::max()) {
                    ThrowStdException(
                        "Unsigned 64-bit integer parameter out"
                        " of range at paramIndex " +
                        std::to_string(paramIndex));
                }
                dataPtr = static_cast<void*>(
                    AllocateParamBuffer<uint64_t>(
                        paramBuffers,
                        param.cast<uint64_t>()));
                break;
            }
            ...
            case SQL_C_TYPE_DATE: {
                ...
                // TODO: can be moved to python by registering
                // SQL_DATE_STRUCT in pybind
            ...
            case SQL_C_NUMERIC: {
                ...
                LOG("Received numeric parameter: precision - "
                    "{}, scale- {}, sign - {}, value - {}",
                    decimalParam.precision,
                    decimalParam.scale,
                    decimalParam.sign,
                    decimalParam.val);
                ...
                std::memset(
                    static_cast<void*>(decimalPtr->val),
                    0,
            ...
            default: {
                ...
                paramInfo.paramCType << " for parameter - " << paramIndex;
    ...
    // https://learn.microsoft.com/en-us/sql/odbc/reference/appendixes/retrieve-numeric-data-sql-numeric-struct-kb222831?view=sql-server-ver16#sql_c_numeric-overview
            if (paramInfo.paramCType == SQL_C_NUMERIC) {
                ...
                rc = SQLGetStmtAttr_ptr(
                    hStmt,
                    SQL_ATTR_APP_PARAM_DESC,
                    &hDesc, 0, NULL);
                if (!SQL_SUCCEEDED(rc)) {
                    LOG(
                        "Error when getting statement attribute - {}",
                        paramIndex);
                    return rc;
                }
                rc = SQLSetDescField_ptr(
                    hDesc, 1, SQL_DESC_TYPE,
                    (SQLPOINTER) SQL_C_NUMERIC, 0);
                if (!SQL_SUCCEEDED(rc)) {
                    LOG(
                        "Error when setting descriptor"
                        " field SQL_DESC_TYPE - {}",
                        paramIndex);
                    return rc;
                }
                ...
                if (!SQL_SUCCEEDED(rc)) {
                    LOG(
                        "Error when setting descriptor field "
                        "SQL_DESC_PRECISION - {}", paramIndex);
                    return rc;
                }
                rc = SQLSetDescField_ptr(
                ...
                if (!SQL_SUCCEEDED(rc)) {
                    LOG(
                        "Error when setting descriptor field "
                        "SQL_DESC_SCALE - {}", paramIndex);
                    return rc;
                }
                rc = SQLSetDescField_ptr(
                    hDesc, 1, SQL_DESC_DATA_PTR, (SQLPOINTER) numericPtr, 0);
                if (!SQL_SUCCEEDED(rc)) {
                    LOG(
                        "Error when setting descriptor field "
                        "SQL_DESC_DATA_PTR - {}", paramIndex);
                    return rc;
                }
            }
    }
    LOG(
        "Finished parameter binding. Number of parameters: {}",
        params.size());
    return SQL_SUCCESS;
}

// This is temporary hack to avoid crash when
// SQLDescribeCol returns 0 as columnSize
// for NVARCHAR(MAX) & similar types.
// Variable length data needs more nuanced handling.
// TODO: Fix this in beta
// This function sets the buffer allocated
// to fetch NVARCHAR(MAX) & similar types to
// 4096 chars. So we'll retrieve data upto 4096.
// Anything greater then that will throw error
...
void LOG(const std::string& formatString, Args&&... args) {
    ...
    py::object logger =
        py::module_::import("mssql_python.logging_config")
        .attr("get_logger")();
...
std::string GetModuleDirectory() {
    ...
    errno_t err = strncpy_s(
        path, MAX_PATH,
        module_file.c_str(), module_file.length());
    if (err != 0) {
...
DriverHandle LoadDriverLibrary(const std::string& driverPath) {
    ...
    if (!handle) {
        LOG(
            ...
            driverPath, GetLastErrorMessage());
        ThrowStdException(
            "Failed to load library: " + driverPath);
    }
    return handle;
#else
...
std::string GetDriverPathCpp(const std::string& moduleDir) {
    ...
    // Detect platform and set path
        ...
        if (fs::exists("/etc/alpine-release")) {
            ...
            fs::exists("/etc/redhat-release") ||
            fs::exists("/etc/centos-release")) {
            platform = "rhel";
        } else if (
            fs::exists("/etc/SuSE-release") ||
            fs::exists("/etc/SUSE-brand")) {
            platform = "suse";
        } else {
            // Default to debian_ubuntu for other distros
            platform = "debian_ubuntu";
        }

        fs::path driverPath =
            basePath / "libs" / "linux" /
            platform / arch / "lib" / "libmsodbcsql-18.5.so.1.1";
        return driverPath.string();

    #elif defined(__APPLE__)
        platform = "macos";
        fs::path driverPath =
            basePath / "libs" / platform /
            arch / "lib" / "libmsodbcsql.18.dylib";
        return driverPath.string();
...
DriverHandle LoadDriverOrThrowException() {
    ...
    // Not using Python function since it
    // causes circular import issues on Alpine Linux
    ...
    LOG("Driver path determined: {}", driverPath.string());
        ...
        if (fs::exists(authDllPath)) {
            std::wstring authDllWidePath(
                authDllPath.native().begin(),
                authDllPath.native().end());
            HMODULE hAuth = LoadLibraryW(authDllWidePath.c_str());
            if (hAuth) {
                LOG(
                    "mssql-auth.dll loaded: {}",
                    authDllPath.string());
            } else {
                LOG(
                ...
                ThrowStdException(
                    "Failed to load mssql-auth.dll. "
                    "Please ensure it is present in the expected directory.");
            }
        } else {
            LOG("Note: mssql-auth.dll not found. "
                "This is OK if Entra ID is not in use.");
            ThrowStdException(
                "mssql-auth.dll not found. "
                "If you are using Entra ID, please ensure it is present.");
        }
    #endif

    if (!fs::exists(driverPath)) {
        ThrowStdException(
            "ODBC driver not found at: " + driverPath.string());
    }
    ...
    SQLAllocHandle_ptr = GetFunctionPointer<SQLAllocHandleFunc>(
        handle, "SQLAllocHandle");
    SQLSetEnvAttr_ptr = GetFunctionPointer<SQLSetEnvAttrFunc>(
        handle, "SQLSetEnvAttr");
    SQLSetConnectAttr_ptr = GetFunctionPointer<SQLSetConnectAttrFunc>(
        handle, "SQLSetConnectAttrW");
    SQLSetStmtAttr_ptr = GetFunctionPointer<SQLSetStmtAttrFunc>(
        handle, "SQLSetStmtAttrW");
    SQLGetConnectAttr_ptr = GetFunctionPointer<SQLGetConnectAttrFunc>(
        handle, "SQLGetConnectAttrW");

    SQLDriverConnect_ptr = GetFunctionPointer<SQLDriverConnectFunc>(
        handle, "SQLDriverConnectW");
    SQLExecDirect_ptr = GetFunctionPointer<SQLExecDirectFunc>(
        handle, "SQLExecDirectW");
    SQLPrepare_ptr = GetFunctionPointer<SQLPrepareFunc>(
        handle, "SQLPrepareW");
    SQLBindParameter_ptr = GetFunctionPointer<SQLBindParameterFunc>(
        handle, "SQLBindParameter");
    SQLExecute_ptr = GetFunctionPointer<SQLExecuteFunc>(
        handle, "SQLExecute");
    SQLRowCount_ptr = GetFunctionPointer<SQLRowCountFunc>(
        handle, "SQLRowCount");
    SQLGetStmtAttr_ptr = GetFunctionPointer<SQLGetStmtAttrFunc>(
        handle, "SQLGetStmtAttrW");
    SQLSetDescField_ptr = GetFunctionPointer<SQLSetDescFieldFunc>(
        handle, "SQLSetDescFieldW");

    SQLFetch_ptr = GetFunctionPointer<SQLFetchFunc>(
        handle, "SQLFetch");
    SQLFetchScroll_ptr = GetFunctionPointer<SQLFetchScrollFunc>(
        handle, "SQLFetchScroll");
    SQLGetData_ptr = GetFunctionPointer<SQLGetDataFunc>(
        handle, "SQLGetData");
    SQLNumResultCols_ptr = GetFunctionPointer<SQLNumResultColsFunc>(
        handle, "SQLNumResultCols");
    SQLBindCol_ptr = GetFunctionPointer<SQLBindColFunc>(
        handle, "SQLBindCol");
    SQLDescribeCol_ptr = GetFunctionPointer<SQLDescribeColFunc>(
        handle, "SQLDescribeColW");
    SQLMoreResults_ptr = GetFunctionPointer<SQLMoreResultsFunc>(
        handle, "SQLMoreResults");
    SQLColAttribute_ptr = GetFunctionPointer<SQLColAttributeFunc>(
        handle, "SQLColAttributeW");

    SQLEndTran_ptr = GetFunctionPointer<SQLEndTranFunc>(
        handle, "SQLEndTran");
    SQLDisconnect_ptr = GetFunctionPointer<SQLDisconnectFunc>(
        handle, "SQLDisconnect");
    SQLFreeHandle_ptr = GetFunctionPointer<SQLFreeHandleFunc>(
        handle, "SQLFreeHandle");
    SQLFreeStmt_ptr = GetFunctionPointer<SQLFreeStmtFunc>(
        handle, "SQLFreeStmt");

    SQLGetDiagRec_ptr = GetFunctionPointer<SQLGetDiagRecFunc>(
        handle, "SQLGetDiagRecW");

    SQLParamData_ptr = GetFunctionPointer<SQLParamDataFunc>(
        handle, "SQLParamData");
    SQLPutData_ptr = GetFunctionPointer<SQLPutDataFunc>(
        handle, "SQLPutData");
    SQLTables_ptr = GetFunctionPointer<SQLTablesFunc>(
        handle, "SQLTablesW");

    SQLDescribeParam_ptr = GetFunctionPointer<SQLDescribeParamFunc>(
        handle, "SQLDescribeParam");
...
void SqlHandle::free() {
    if (_handle && SQLFreeHandle_ptr) {
        ...
        _handle = nullptr;
        // Don't log during destruction -
        // it can cause segfaults during Python shutdown
    }
}

// Helper function to check for driver errors
ErrorInfo SQLCheckError_Wrap(
    SQLSMALLINT handleType, SqlHandlePtr handle, SQLRETURN retcode) {
    ...
    if (!SQL_SUCCEEDED(retcode)) {
        ...
        SQLRETURN diagReturn =
            SQLGetDiagRec_ptr(
                handleType, rawHandle, 1, sqlState,
                &nativeError, message,
...
#else
            // On macOS/Linux, need to convert SQLWCHAR
            // (usually unsigned short) to wchar_t
...
py::list SQLGetAllDiagRecords(SqlHandlePtr handle) {
    ...
    for (SQLSMALLINT recNumber = 1; ; recNumber++) {
        ...
        int stateSize = WideCharToMultiByte(
            CP_UTF8, 0, sqlState, -1, NULL, 0, NULL, NULL);
        std::vector<char> stateBuffer(stateSize);
        WideCharToMultiByte(
            CP_UTF8, 0, sqlState, -1, stateBuffer.data(),
            stateSize, NULL, NULL);

        // Format the state with error code
        std::string stateWithError =
            "[" + std::string(stateBuffer.data()) +
            "] (" + std::to_string(nativeError) + ")";

        // Convert wide string message to UTF-8
        int msgSize = WideCharToMultiByte(
            CP_UTF8, 0, message, -1, NULL, 0, NULL, NULL);
        std::vector<char> msgBuffer(msgSize);
        WideCharToMultiByte(
            CP_UTF8, 0, message, -1, msgBuffer.data(),
            msgSize, NULL, NULL);
...
#else
        // On Unix, use the SQLWCHARToWString
        // utility and then convert to UTF-8
        std::string stateStr = WideToUTF8(SQLWCHARToWString(sqlState));
        std::string msgStr = WideToUTF8(
            SQLWCHARToWString(message, messageLen));
...
SQLRETURN SQLExecDirect_wrap(
    SqlHandlePtr StatementHandle, const std::wstring& Query) {
    ...
    SQLRETURN ret = SQLExecDirect_ptr(
        StatementHandle->get(), queryPtr, SQL_NTS);
...
// Executes the provided query. If the query is parametrized,
// it prepares the statement and binds the parameters.
// Otherwise, it executes the query directly.
// 'usePrepare' parameter can be used to disable
// the prepare step for queries that might already
...
SQLRETURN SQLExecute_wrap(const SqlHandlePtr statementHandle,
    ...
    if (!SQLPrepare_ptr) {
        LOG("Function pointer not initialized. "
            "Loading the driver.");
        // Load the driver
        DriverLoader::getInstance().loadDriver();
    }
    ...
    if (params.size() != paramInfos.size()) {
        // TODO: This should be a special internal exception,
        // that python wont relay to users as is
...
#endif
    if (params.size() == 0) {
        // Execute statement directly if the statement is not parametrized.
        // This is the fastest way to submit a SQL statement
        // for one-time execution according to
        // DDBC documentation -
        ...
        // isStmtPrepared is a list instead of a bool coz bools in
        // Python are immutable.
        // Hence, we can't pass around bools by reference & modify them.
        // Therefore, isStmtPrepared
        ...
        if (usePrepare) {
            ...
            // Make sure the statement has been prepared
            // earlier if we're not preparing now
        ...
        // This vector manages the heap memory
        // allocated for parameter buffers.
                ...
                if (py::isinstance<py::str>(pyObj)) {
                    if (matchedInfo->paramCType == SQL_C_WCHAR) {
                        ...
                        while (offset < totalChars) {
                            ...
                            if (!SQL_SUCCEEDED(rc)) {
                                LOG(
                                    "SQLPutData failed at offset {} of {}",
                                    offset, totalChars);
                                return rc;
                            }
                            offset += len;
        ...
        if (!SQL_SUCCEEDED(rc) && rc != SQL_NO_DATA) {
            LOG("DDBCSQLExecute: "
                "Error during execution of the statement");
            return rc;
        }

        // Unbind the bound buffers for
        // all parameters coz the buffers' memory will
        // be freed when this function exits
        // (parambuffers goes out of scope)
...
SQLRETURN BindParameterArray(SQLHANDLE hStmt,
                             ...
                             size_t paramSetSize,
                             std::vector<std::shared_ptr<void>>& paramBuffers) {
    LOG("Starting column-wise parameter array binding."
        " paramSetSize: {}, paramCount: {}",
    ...
    try {
        for (
          int paramIndex = 0;
          paramIndex < columnwise_params.size();
          ++paramIndex) {
            ...
            switch (info.paramCType) {
                ...
                case SQL_C_DOUBLE: {
                    double* dataArray =
                        AllocateParamBufferArray<double>(
                            tempBuffers, paramSetSize);
                ...
                case SQL_C_WCHAR: {
                    SQLWCHAR* wcharArray =
                        AllocateParamBufferArray<SQLWCHAR>(
                            tempBuffers,
                            paramSetSize * (info.columnSize + 1));
                    strLenOrIndArray =
                        AllocateParamBufferArray<SQLLEN>(
                            tempBuffers, paramSetSize);
                    for (size_t i = 0; i < paramSetSize; ++i) {
                        if (columnValues[i].is_none()) {
                            ...
                            // then check the actual UTF-16 length
                            ...
                            // If we reach here, the UTF-16 string fits
                            // - copy it completely
                            std::memcpy(
                                wcharArray + i * (info.columnSize + 1),
                                utf16Buf.data(),
                            ...
                            if (wstr.length() > info.columnSize) {
                                std::string offending = WideToUTF8(wstr);
                                ThrowStdException(
                                    "Input string exceeds allowed column"
                                    " size at parameter index " +
                                    std::to_string(paramIndex));
                            }
                            std::memcpy(
                                wcharArray + i * (info.columnSize + 1),
                ...
                case SQL_C_UTINYINT: {
                    unsigned char* dataArray =
                        AllocateParamBufferArray<unsigned char>(
                            tempBuffers, paramSetSize);
                    for (size_t i = 0; i < paramSetSize; ++i) {
                        if (columnValues[i].is_none()) {
                            if (!strLenOrIndArray)
                ...
                case SQL_C_SHORT: {
                    int16_t* dataArray =
                        AllocateParamBufferArray<int16_t>(
                            tempBuffers, paramSetSize);
                    for (size_t i = 0; i < paramSetSize; ++i) {
                        if (columnValues[i].is_none()) {
                            if (!strLenOrIndArray)
                ...
                case SQL_C_USHORT: {
                    uint16_t* dataArray =
                        AllocateParamBufferArray<uint16_t>(
                            tempBuffers, paramSetSize);
                    strLenOrIndArray =
                        AllocateParamBufferArray<SQLLEN>(
                            tempBuffers, paramSetSize);
                ...
                case SQL_C_ULONG: {
                    int64_t* dataArray =
                        AllocateParamBufferArray<int64_t>(
                            tempBuffers, paramSetSize);
                    strLenOrIndArray =
                        AllocateParamBufferArray<SQLLEN>(
                            tempBuffers, paramSetSize);
                ...
                case SQL_C_FLOAT: {
                    float* dataArray =
                        AllocateParamBufferArray<float>(
                            tempBuffers, paramSetSize);
                    strLenOrIndArray =
                        AllocateParamBufferArray<SQLLEN>(
                            tempBuffers, paramSetSize);
                ...
                case SQL_C_TYPE_DATE: {
                    SQL_DATE_STRUCT* dateArray =
                        AllocateParamBufferArray<SQL_DATE_STRUCT>(
                            tempBuffers, paramSetSize);
                    strLenOrIndArray =
                        AllocateParamBufferArray<SQLLEN>(
                            tempBuffers, paramSetSize);
                ...
                case SQL_C_TYPE_TIME: {
                    SQL_TIME_STRUCT* timeArray =
                        AllocateParamBufferArray<SQL_TIME_STRUCT>(
                            tempBuffers, paramSetSize);
                    strLenOrIndArray =
                        AllocateParamBufferArray<SQLLEN>(
                            tempBuffers, paramSetSize);
                    ...
                    for (size_t i = 0; i < paramSetSize; ++i) {
                        if (columnValues[i].is_none()) {
                            ...
                            tsArray[i].fraction = static_cast<SQLUINTEGER>(
                                dtObj.attr("microsecond").cast<int>() * 1000);
                            strLenOrIndArray[i] = 0;
                        }
                    }
                    dataPtr = tsArray;
                ...
                case SQL_C_NUMERIC: {
                    ...
                    for (size_t i = 0; i < paramSetSize; ++i) {
                        ...
                        LOG(
                            "Received numeric parameter at [%zu]:"
                            "precision=%d, scale=%d, sign=%d, val=%lld",
                            i, decimalParam.precision,
                        ...
                        std::memset(numericArray[i].val, 0,
                            sizeof(numericArray[i].val));
                        std::memcpy(numericArray[i].val,
                            reinterpret_cast<const char*>(
                                &decimalParam.val),
                            std::min(sizeof(decimalParam.val),
                            sizeof(numericArray[i].val)));
        ...
        LOG("Exception occurred during parameter "
            "array binding. Cleaning up.");
        throw;
    }
    paramBuffers.insert(
        paramBuffers.end(),
...
SQLRETURN SQLExecuteMany_wrap(const SqlHandlePtr statementHandle,
    ...
    std::vector<std::shared_ptr<void>> paramBuffers;
    rc = BindParameterArray(
        hStmt, columnwise_params,
        paramInfos, paramSetSize,
        paramBuffers);
    if (!SQL_SUCCEEDED(rc)) return rc;
    rc = SQLSetStmtAttr_ptr(
        hStmt, SQL_ATTR_PARAMSET_SIZE,
    ...
    for (SQLUSMALLINT i = 1; i <= ColumnCount; ++i) {
        ...
        if (SQL_SUCCEEDED(retcode)) {
            ...
            ColumnMetadata.append(
                py::dict("ColumnName"_a =
                    SQLWCHARToWString(ColumnName, SQL_NTS),
#else
            ColumnMetadata.append(
                py::dict("ColumnName"_a =
...
SQLRETURN SQLGetData_wrap(
    SqlHandlePtr StatementHandle,
    SQLUSMALLINT colCount, py::list& row) {
    ...
    for (SQLSMALLINT i = 1; i <= colCount; ++i) {
        ...
        switch (dataType) {
            ...
            case SQL_LONGVARCHAR: {
                ...
                // null terminator
                uint64_t fetchBufferSize = columnSize + 1;
                ...
                if (SQL_SUCCEEDED(ret)) {
                    // TODO: Refactor these if's across
                    // other switches to avoid code duplication
                    // columnSize is in chars, dataLen is in bytes
                    if (dataLen > 0) {
                        uint64_t numCharsInData =
                            dataLen / sizeof(SQLCHAR);
                        // NOTE: dataBuffer.size() includes
                        // null-terminator, dataLen doesn't.
                        // Hence use '<'.
                        if (numCharsInData < dataBuffer.size()) {
                            ...
                            row.append(fullStr);
                            LOG(
                                "macOS/Linux: Appended CHAR string of "
                                "length {} to result row",
                                fullStr.length());
#else
                            row.append(
                                std::string(
                            ...
                            oss << "Buffer length for fetch (" <<
                                dataBuffer.size()-1 <<
                                ") is smaller, & data "
                                << "to be retrieved is longer ("
                                << numCharsInData << "). ColumnID - "
                                << i << ", datatype - "
                                << dataType;
                        ...
                        // This means the length of the
                        // data couldn't be determined
                        LOG("SQLGetData couldn't determine "
                            "the length of the data. "
                            "Returning NULL value instead."
                            " Column ID - {}, Data Type - {}", i, dataType);
                    } else if (dataLen < 0) {
                        // This is unexpected
                        LOG("SQLGetData returned an unexpected"
                            " negative data length. "
                            "Raising exception. Column ID - {},"
                            " Data Type - {}, Data Length - {}",
                            i, dataType, dataLen);
                        ThrowStdException(
                            "SQLGetData returned an "
                            "unexpected negative data length");
                    }
                } else {
                    LOG("Error retrieving data for column - {},"
                        " data type - {}, SQLGetData return "
            ...
            case SQL_WLONGVARCHAR: {
                ...
                ret = SQLGetData_ptr(
                    hStmt, i, SQL_C_WCHAR,
                    dataBuffer.data(),
                    dataBuffer.size() * sizeof(SQLWCHAR),
                    &dataLen);

                if (SQL_SUCCEEDED(ret)) {
                    // TODO: Refactor these if's across other
                    // switches to avoid code duplication
                    if (dataLen > 0) {
                        uint64_t numCharsInData =
                        ...
                        if (numCharsInData < dataBuffer.size()) {
                            ...
                            if (dataLen < 0 ||
                                ...
                                actualBufferSize) {
                                LOG(
                                    "Error: py::bytes creation request"
                                    " exceeds buffer size. dataLen={} "
                                    "buffer={}",
                                    dataLen, actualBufferSize);
                                ThrowStdException(
                                    "Invalid buffer length for py::bytes");
                            }
                            py::bytes py_bytes(raw_bytes, dataLen);
                            py::str decoded =
                                py_bytes.attr("decode")("utf-16-le");
                            row.append(decoded);
#else
                            ...
                            std::ostringstream oss;
                            oss << "Buffer length for fetch (" <<
                                dataBuffer.size()-1 << ") is smaller, & data "
                                << "to be retrieved is longer (" <<
                                numCharsInData << "). ColumnID - "
                                << i << ", datatype - " << dataType;
                            ThrowStdException(oss.str());
            ...
            case SQL_NUMERIC: {
                ...
                ret = SQLGetData_ptr(
                    hStmt, i, SQL_C_CHAR,
                    numericStr, sizeof(numericStr),
                    &indicator);

                if (SQL_SUCCEEDED(ret)) {
                    try {
                    // Convert numericStr to py::decimal.
                    // Decimal and append to row
                    row.append(
            ...
            case SQL_TYPE_DATE: {
                ...
                ret =
                    SQLGetData_ptr(
                        hStmt, i, SQL_C_TYPE_DATE,
                        &dateValue, sizeof(dateValue),
                        NULL);
                if (SQL_SUCCEEDED(ret)) {
                    row.append(
                        py::module_::import("datetime").attr("date")(
            ...
            case SQL_SS_TIME2: {
                ...
                ret =
                    SQLGetData_ptr(
                        hStmt, i, SQL_C_TYPE_TIME,
                        &timeValue, sizeof(timeValue),
                        NULL);
                if (SQL_SUCCEEDED(ret)) {
                    row.append(
                        py::module_::import("datetime").attr("time")(
                ...
                if (SQL_SUCCEEDED(ret)) {
                    row.append(
                        py::module_::import("datetime").attr("datetime")(
                            ...
                            // Convert back ns to µs
                            timestampValue.fraction / 1000));
                } else {
                    LOG("Error retrieving data for column - {},"
                        " data type - {}, SQLGetData return "
                        "code - {}. Returning NULL value instead",
            ...
            case SQL_LONGVARBINARY: {
                ...
                SQLLEN dataLen;
                ret = SQLGetData_ptr(
                    hStmt, i, SQL_C_BINARY,
                    dataBuffer.get(), columnSize,
                    &dataLen);
            ...
            case SQL_GUID: {
                ...
                if (SQL_SUCCEEDED(ret)) {
                    ...
                    oss << std::hex << std::setfill('0')
                        ...
                        << std::setw(2)
                        << static_cast<int>(guidValue.Data4[0])
                        << std::setw(2)
                        << static_cast<int>(guidValue.Data4[1])
                        << '-' << std::hex << std::setw(2)
                        << static_cast<int>(guidValue.Data4[2])
                        << std::setw(2)
                        << static_cast<int>(guidValue.Data4[3])
                        << std::setw(2)
                        << static_cast<int>(guidValue.Data4[4])
                        << std::setw(2)
                        << static_cast<int>(guidValue.Data4[5])
                        << std::setw(2)
                        << static_cast<int>(guidValue.Data4[6])
                        << std::setw(2)
...
SQLRETURN SQLFetchScroll_wrap(
    SqlHandlePtr StatementHandle,
    SQLSMALLINT FetchOrientation,
    SQLLEN FetchOffset,
    py::list& row_data) {
    LOG("Fetching with scroll: orientation={}, offset={}",
    ...
    // Unbind any columns from previous
    // fetch operations to avoid memory corruption
    SQLFreeStmt_ptr(StatementHandle->get(), SQL_UNBIND);

    // Perform scroll operation
    SQLRETURN ret = SQLFetchScroll_ptr(
                        StatementHandle->get(),
                        FetchOrientation, FetchOffset);
...
SQLRETURN SQLBindColums(SQLHSTMT hStmt, ColumnBuffers& buffers,
    ...
    for (SQLUSMALLINT col = 1; col <= numCols; col++) {
        ...
        SQLSMALLINT dataType =
            columnMeta["DataType"].cast<SQLSMALLINT>();
        SQLULEN columnSize =
        ...
        switch (dataType) {
            ...
            case SQL_LONGVARCHAR: {
                // TODO: handle variable length data correctly.
                // This logic wont suffice
                HandleZeroColumnSizeAtFetch(columnSize);
                // null-terminator
                uint64_t fetchBufferSize = columnSize + 1;
        // TODO: For LONGVARCHAR/BINARY types,
        // columnSize is returned as 2GB-1 by
        // SQLDescribeCol. So fetchBufferSize = 2GB.
        // fetchSize=1 if columnSize>1GB.
        // So we'll allocate a vector of size 2GB.
        // If a query fetches multiple (say N)
        // LONG... columns, we will have
        // allocated multiple (N) 2GB sized vectors.
        // This will make driver very slow. And if the N is high enough,
        // we could hit the OS
        // limit for heap memory that we can allocate,
        // & hence get a std::bad_alloc.
        // The process could also be killed by OS for consuming too much memory.
        // Hence this will be revisited in beta to not allocate 2GB+ memory,
        // & use streaming instead
                buffers.charBuffers[col - 1].resize(
                    fetchSize * fetchBufferSize);
                ret = SQLBindCol_ptr(
            ...
            case SQL_WLONGVARCHAR: {
                ...
                // null-terminator
                uint64_t fetchBufferSize = columnSize + 1;
                buffers.wcharBuffers[col - 1].resize(
                    fetchSize * fetchBufferSize);
                ret = SQLBindCol_ptr(
                        hStmt, col, SQL_C_WCHAR,
            ...
            case SQL_NUMERIC:
                buffers.charBuffers[col - 1].resize(
                    fetchSize * MAX_DIGITS_IN_NUMERIC);
                ret = SQLBindCol_ptr(
                        hStmt, col, SQL_C_CHAR,
                        buffers.charBuffers[col - 1].data(),
            ...
            case SQL_LONGVARBINARY:
                ...
                HandleZeroColumnSizeAtFetch(columnSize);
                buffers.charBuffers[col - 1].resize(
                    fetchSize * columnSize);
                ret = SQLBindCol_ptr(
                        hStmt, col, SQL_C_BINARY,
                        buffers.charBuffers[col - 1].data(),
                        columnSize,
                        buffers.indicators[col - 1].data());
                break;
        ...
        if (!SQL_SUCCEEDED(ret)) {
            std::wstring columnName =
                columnMeta["ColumnName"].cast<std::wstring>();
            std::ostringstream errorString;
            errorString << "Failed to bind column - "
                        << columnName.c_str()
                        << ", Type - " << dataType
                        << ", column ID - " << col;
            LOG(errorString.str());
...
// since it is not used outside this file
SQLRETURN FetchBatchData(
    SQLHSTMT hStmt, ColumnBuffers& buffers,
    py::list& columnNames,
    py::list& rows, SQLUSMALLINT numCols,
    ...
    // It'll be populated by SQLFetchScroll
    for (SQLULEN i = 0; i < numRowsFetched; i++) {
        ...
        for (SQLUSMALLINT col = 1; col <= numCols; col++) {
            ...
            // TODO: variable length data needs special handling,
            // this logic wont suffice
            // This value indicates that the driver
            // cannot determine the length of the data
            if (dataLen == SQL_NO_TOTAL) {
                LOG("Cannot determine the length of the data. "
                    "Returning NULL value instead."
                    "Column ID - {}", col);
                row.append(py::none());
                continue;
                ...
                LOG(
                    "Column data is NULL."
                    " Appending None to the result row."
                    " Column ID - {}",
                    col);
                row.append(py::none());
                continue;
                ...
                if (
                    ...
                    // For other datatypes, 0 length is unexpected.
                    // Log & append None
                    LOG(
                        "Column data length is 0 for "
                        "non-string/binary datatype."
                        " Appending None to the result row."
                        " Column ID - {}",
                        col);
                    row.append(py::none());
                ...
                // log column index, SQL type & raise exception
                LOG(
                    "Unexpected negative data length."
                    " Column ID - {}, SQL Type - {}, Data Length - {}",
            ...
            switch (dataType) {
                ...
                case SQL_LONGVARCHAR: {
                    ...
                    // fetchBufferSize includes null-terminator,
                    // numCharsInData doesn't. Hence '<'
                    if (numCharsInData < fetchBufferSize) {
                        // SQLFetch will nullterminate the data
                        row.append(std::string(
                         reinterpret_cast<char*>(
                         &buffers.charBuffers[col - 1][i * fetchBufferSize]),
                         numCharsInData));
                    } else {
                        // In this case, buffer size is smaller,
                        // and data to be retrieved is longer
                ...
                case SQL_WVARCHAR:
                case SQL_WLONGVARCHAR: {
                    // TODO: variable length data needs special
                    // handling, this logic wont suffice
                    SQLULEN columnSize =
                        columnMeta["ColumnSize"].cast<SQLULEN>();
                    HandleZeroColumnSizeAtFetch(columnSize);
                    // null-terminator
                    ...
                    if (numCharsInData < fetchBufferSize) {
                        // SQLFetch will nullterminate the data
#if defined(__APPLE__) || defined(__linux__)
                        // Use unix-specific conversion to handle
                        // the wchar_t/SQLWCHAR size difference
                        SQLWCHAR* wcharData =
                        ...
                        row.append(wstr);
#else
                        // On Windows, wchar_t and SQLWCHAR are both 2 bytes,
                        // so direct cast works
                        row.append(
                          std::wstring(
                ...
                case SQL_INTEGER: {
                    row.append(
                        buffers.intBuffers[col - 1][i]);
                    break;
                }
                case SQL_SMALLINT: {
                    row.append(
                        buffers.smallIntBuffers[col - 1][i]);
                    break;
                }
                case SQL_TINYINT: {
                    row.append(
                        buffers.charBuffers[col - 1][i]);
                    break;
                }
                case SQL_BIT: {
                    row.append(static_cast<bool>(
                        buffers.charBuffers[col - 1][i]));
                    break;
                }
                case SQL_REAL: {
                ...
                case SQL_NUMERIC: {
                    try {
                     ...
                     row.append(
                        ...
                        // log the error and append py::none()
                        LOG("Error converting to decimal: {}", e.what());
                        row.append(py::none());
                    }
                    break;
                ...
                case SQL_LONGVARBINARY: {
                    ...
                    if (static_cast<size_t>(dataLen) <= columnSize) {
                        ...
                        // TODO: Revisit
                        std::ostringstream oss;
                        oss << "Buffer length for fetch (" << columnSize <<
                            ") is smaller, & data " <<
                            "to be retrieved is longer ("
                            << dataLen << "). ColumnID - " << col <<
                            ", datatype - " << dataType;
                        ThrowStdException(oss.str());
                    }
                    break;
...
// Given a list of columns that are a part of single
// row in the result set, calculates
...
//
// @param StatementHandle: Handle to the statement from
//                         which data is to be fetched.
// @param rows: A Python list that will be populated
//              with the fetched rows of data.
...
//
// This function assumes that the statement handle (hStmt)
// is already allocated and a query has been
// executed. It fetches the specified number of rows
// from the result set and populates the provided
// Python list with the row data.
// If there are no more rows to fetch, it returns SQL_NO_DATA.
// If an error occurs during fetching, it throws a runtime error.
SQLRETURN FetchMany_wrap(SqlHandlePtr StatementHandle,
    py::list& rows, int fetchSize = 1) {
    SQLRETURN ret;
    SQLHSTMT hStmt = StatementHandle->get();
    ...
    ret = FetchBatchData(hStmt, buffers, columnNames,
        rows, numCols, numRowsFetched);
...
//
// @param StatementHandle: Handle to the statement
//                         from which data is to be fetched.
// @param rows: A Python list that will be
//              populated with the fetched rows of data.
//
// @return SQLRETURN: SQL_SUCCESS if data is fetched successfully,
//                    SQL_NO_DATA if there are no more rows to fetch,
//                    throws a runtime error if there is an error fetching data.
//
// This function assumes that the statement handle (hStmt) is
// already allocated and a query has been
// executed. It fetches all rows from the result set and
// populates the provided Python list with the
// row data. If there are no more rows to fetch,
// it returns SQL_NO_DATA. If an error occurs during
...
SQLRETURN FetchAll_wrap(SqlHandlePtr StatementHandle, py::list& rows) {
    ...
    // Even though we're fetching fetchSize rows at a time,
    // fetchall will keep all rows in memory anyway.
    // So what are we gaining by fetching fetchSize rows at a time?
    // Also, say the table has only 10 rows, each row size if 100 bytes.
    // Here, we'll have fetchSize = 1000, so we'll allocate memory for
    // 1000 rows inside SQLBindCol_wrap, while actually only need to
    // retrieve 10 rows
    int fetchSize;
    if (numRowsInMemLimit == 0) {
        // If the row size is larger than the memory limit,
    ...
    while (ret != SQL_NO_DATA) {
        ret = FetchBatchData(hStmt, buffers,
            columnNames, rows, numCols, numRowsFetched);
        if (!SQL_SUCCEEDED(ret) && ret != SQL_NO_DATA) {
            LOG("Error when fetching data");
            return ret;
        }
...
//
// @return SQLRETURN: SQL_SUCCESS or SQL_SUCCESS_WITH_INFO
...
// This function assumes that the statement handle (hStmt)
// is already allocated and a query has been
// executed. It fetches the next row of data from the result
// set and populates the provided Python
// list with the row data. If there are no more rows to fetch,
// it returns SQL_NO_DATA. If an error
...
PYBIND11_MODULE(ddbc_bindings, m) {
    ...
    py::class_<ConnectionHandle>(m, "Connection")
        .def(
            py::init<const std::string&, bool, const py::dict&>(),
            py::arg("conn_str"),
            py::arg("use_pool"),
    ...
    m.def("DDBCSQLFetchMany", &FetchMany_wrap,
    ...
    m.def("DDBCSQLFetchScroll", &SQLFetchScroll_wrap,
          "Scroll to a specific position in the result "
          "set and optionally fetch data");
    ...
    try {
        ...
        LOG(
            "Failed to load ODBC driver during module initialization: {}",
            e.what());
    }
}
