                    ...
                    if (matchedInfo->paramCType == SQL_C_WCHAR) {
                        ...
                        while (offset < totalChars) {
                            ...
                            if (
                                ...
                                ThrowStdException(
                                    "Chunk size exceeds maximum allowed by SQLLEN");
                            }
                            rc = SQLPutData_ptr(
                                    hStmt,
                    ...
                    if (dataLen > 0) {
                        ...
                        if (numCharsInData < dataBuffer.size()) {
                            ...
                            size_t actualBufferSize =
                                dataBuffer.size() * sizeof(SQLWCHAR);
                            if (dataLen < 0 ||
                                static_cast<size_t>(dataLen) > actualBufferSize) {
                                LOG(
                        ...
                        assert(dataLen == SQL_NO_TOTAL);
                        LOG("SQLGetData couldn't determine the"
                            " length of the data. "
                            "Returning NULL value instead. Column ID"
                            " - {}", i);
                        row.append(py::none());
                    }
            } else {
...
SQLRETURN SQLBindColums(SQLHSTMT hStmt, ColumnBuffers& buffers,
    ...
    for (SQLUSMALLINT col = 1; col <= numCols; col++) {
        ...
        // LONG... columns, we will have allocated multiple (N) 2GB sized vectors.
        // This will make driver very slow. And if the N is high enough,
        // we could hit the OS
        // limit for heap memory that we can allocate, & hence get a std::bad_alloc.
    ...
    for (SQLULEN i = 0; i < numRowsFetched; i++) {
        ...
        for (SQLUSMALLINT col = 1; col <= numCols; col++) {
            ...
            assert(
                ...
                "SQL_NO_DATA is already handled");
