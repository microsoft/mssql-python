...
ConnectionPool::ConnectionPool(const std::wstring& conn_str, size_t max_size, int idle_timeout_secs)
    : _conn_str(conn_str), _max_size(max_size),  _idle_timeout_secs(idle_timeout_secs) {
...
std::shared_ptr<Connection> ConnectionPool::acquire() {
    ...
    // std::cout << "[POOL] Acquiring connection. Pool size: " << _pool.size() <<  "\n";
    ...
    while (!_pool.empty()) {
        ...
        // std::cout << "[POOL] Checking connection status...\n";
        if (conn->isAlive()) {
            // std::cout << "[POOL] Reusing alive connection.\n";
            if (!conn->reset()) {
                LOG("Pooled connection reset failed, skipping.");
                continue;
            }
            // conn->updateLastUsed();
            return conn;
    ...
    // if (_current_size < _max_size) {
        ...
        auto conn = std::make_shared<Connection>(_conn_str, true); // false → real connection
        conn->connect();
        return conn;
...
std::shared_ptr<Connection> ConnectionPoolManager::acquireConnection(const std::wstring& conn_str) {
    ...
    if (it == _pools.end()) {
        ...
        auto pool = std::make_shared<ConnectionPool>(conn_str);
