...
// Add these new methods for bcp_bind and bcp_sendrow
...
SQLRETURN BCPWrapper::bind_column(const py::object& data, 
                                  ...
                                  int server_col_idx) {
    if (!_bcp_initialized || _bcp_finished) {
        std::cout << "BCPWrapper Warning: bind_column called in invalid state." << std::endl;
        throw std::runtime_error("BCPWrapper: bind_column called in invalid state.");
    }
    ...
    try {
        ...
        if (py::isinstance<py::str>(data)) {
            ...
            std::cout << "BCPWrapper Warning: Unknown data type, attempting to convert to string" << std::endl;
    ...
    std::cout << "BCPWrapper: Calling bcp_bind for column " << server_col_idx 
              << ", data_type " << data_type
    ...
    std::cout << "BCPWrapper: bcp_bind successful for column " << server_col_idx << std::endl;
...
SQLRETURN BCPWrapper::send_row() {
    if (!_bcp_initialized || _bcp_finished) {
        std::cout << "BCPWrapper Warning: send_row called in invalid state." << std::endl;
        throw std::runtime_error("BCPWrapper: send_row called in invalid state.");
    }
    
    std::cout << "BCPWrapper: Calling bcp_sendrow" << std::endl;
    SQLRETURN ret = BCPSendRow_ptr(_hdbc);

    std::cout << "BCPWrapper: bcp_sendrow returned " << ret << std::endl;

    if (ret == SQL_NO_DATA) {
        std::cout << "BCPWrapper: bcp_sendrow returned SQL_NO_DATA, indicating no more rows to send." << std::endl;
        return SQL_NO_DATA; // No more rows to send
    }
    if (ret == SQL_SUCCESS_WITH_INFO) {
        std::cout << "BCPWrapper: bcp_sendrow returned SQL_SUCCESS_WITH_INFO, indicating a warning occurred." << std::endl;
    ...
    std::cout << "BCPWrapper: bcp_sendrow successful." << std::endl;
