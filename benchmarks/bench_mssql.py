...
CONNECTION_STRING = "Driver={ODBC Driver 18 for SQL Server};" + os.getenv("DB_CONNECTION_STRING")
...
def setup():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("IF OBJECT_ID('perfbenchmark_child_table', 'U') IS NOT NULL DROP TABLE perfbenchmark_child_table")
        cursor.execute("IF OBJECT_ID('perfbenchmark_parent_table', 'U') IS NOT NULL DROP TABLE perfbenchmark_parent_table")
        cursor.execute("IF OBJECT_ID('perfbenchmark_table', 'U') IS NOT NULL DROP TABLE perfbenchmark_table")
        cursor.execute("IF OBJECT_ID('perfbenchmark_stored_procedure', 'P') IS NOT NULL DROP PROCEDURE perfbenchmark_stored_procedure")
        cursor.execute("CREATE TABLE perfbenchmark_table (id INT, name NVARCHAR(50), age INT)")
        cursor.execute("CREATE TABLE perfbenchmark_parent_table (id INT PRIMARY KEY, name NVARCHAR(50))")
        cursor.execute("CREATE TABLE perfbenchmark_child_table (id INT PRIMARY KEY, parent_id INT, description NVARCHAR(100), FOREIGN KEY (parent_id) REFERENCES perfbenchmark_parent_table(id))")
        cursor.execute("CREATE PROCEDURE perfbenchmark_stored_procedure AS BEGIN SELECT * FROM perfbenchmark_table; END")
        cursor.execute("INSERT INTO perfbenchmark_table (id, name, age) VALUES (1, 'John Doe', 30)")
        conn.commit()
    finally:
        cursor.close()
...
def teardown():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DROP TABLE IF EXISTS perfbenchmark_child_table")
        cursor.execute("DROP TABLE IF EXISTS perfbenchmark_parent_table")
        cursor.execute("DROP TABLE IF EXISTS perfbenchmark_table")
        cursor.execute("DROP PROCEDURE IF EXISTS perfbenchmark_stored_procedure")
        conn.commit()
    finally:
        cursor.close()
        conn.close()
...
def bench_insert_pyodbc():
    conn = pyodbc.connect(CONNECTION_STRING)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO perfbenchmark_table (id, name, age) VALUES (2, 'Jane Doe', 25)")
    conn.commit()
    cursor.close()
    conn.close()
...
def bench_update_pyodbc():
    conn = pyodbc.connect(CONNECTION_STRING)
    cursor = conn.cursor()
    cursor.execute("UPDATE perfbenchmark_table SET age = 35 WHERE id = 2")
    conn.commit()
    cursor.close()
    conn.close()
...
def bench_insert_mssql_python():
    conn = mssql_python.connect(CONNECTION_STRING)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO perfbenchmark_table (id, name, age) VALUES (3, 'Alan', 28)")
    conn.commit()
    cursor.close()
    conn.close()
...
def bench_update_mssql_python():
    conn = mssql_python.connect(CONNECTION_STRING)
    cursor = conn.cursor()
    cursor.execute("UPDATE perfbenchmark_table SET age = 40 WHERE id = 3")
    conn.commit()
    cursor.close()
    conn.close()
